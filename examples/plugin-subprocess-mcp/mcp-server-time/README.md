...
Tiny demo Model Context Protocol (MCP) server. Used by the `plugin-subprocess-mcp` ApexRouter example.
...
This crate is **not** a member of the main ApexRouter workspace. It is a downstream consumer — exactly the shape a third-party MCP server would have.
