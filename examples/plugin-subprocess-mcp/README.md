# Example: wrap an MCP server as a ApexRouter plugin

This example demonstrates the **"wrap an MCP server, get a ApexRouter plugin for free"** story introduced in v0.6.5.

The ApexRouter engine ships with an `mcp-bridge` plugin runtime. Drop a manifest like the one in this directory into `~/.apexrouter/plugins/<name>/plugin.toml` and the engine will:
...
3. Synthesize one ApexRouter `PluginTool` per MCP tool the server advertises.
...
├── plugin.toml                    <- ApexRouter manifest (kind = "mcp-bridge")
...
The `mcp-server-time` crate is intentionally **outside** the main ApexRouter workspace — it represents a downstream third-party MCP server that knows nothing about ApexRouter.
...
cp plugin.toml ~/.apexrouter/plugins/mcp-time/
```

### 3. Adjust `binary_path` to an absolute path

Open `~/.apexrouter/plugins/mcp-time/plugin.toml` and change `runtime.subprocess.binary_path` to the absolute path of the built binary, for example:

```toml
[runtime.subprocess]
binary_path = "/Users/you/dev/apexrouter/examples/plugin-subprocess-mcp/mcp-server-time/target/release/mcp-server-time"
args = []
```

The default value in the shipped manifest (`./mcp-server-time/target/release/mcp-server-time`) is relative to the manifest directory — it only resolves correctly if you run ApexRouter directly out of this examples folder, which most users won't.

### 4. Restart the ApexRouter engine
...
The engine's plugin loader (v0.6.5 Task 2.7b) walks `~/.apexrouter/plugins/*/plugin.toml` at startup. For each manifest:
