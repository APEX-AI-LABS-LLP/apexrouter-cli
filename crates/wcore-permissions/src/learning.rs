...
//!
//! Persistence is TOML at `~/.apexrouter/permissions.toml` (path is
...
impl LearnedPolicy {
    ...
    /// Resolve the default on-disk path (`~/.apexrouter/permissions.toml`).
    pub fn default_path() -> Result<PathBuf, LearningError> {
        dirs::home_dir()
            .map(|h| h.join(".apexrouter").join("permissions.toml"))
