...
//! under `$APEXROUTER_CLI_HOME/skills/auto/<name>/` must be discoverable by the
//! loader on the next session, so a skill "learned" in session 1 is usable in
//! session 2.
//!
//! These tests pin `$APEXROUTER_CLI_HOME` at a tempdir (hermetic) and assert that the
...
//! `APEXROUTER_CLI_HOME` through `app_config_dir`).
...
/// Pin `$APEXROUTER_CLI_HOME` for the test body and restore the prior value on drop.
/// Env mutation is `unsafe`; the `#[serial]` attribute guarantees no other
/// thread observes the mutated env concurrently.
struct ApexRouterHomeGuard {
    prev: Option<String>,
}
impl ApexRouterHomeGuard {
    fn set(dir: &Path) -> Self {
        let prev = std::env::var("APEXROUTER_CLI_HOME").ok();
        unsafe {
            std::env::set_var("APEXROUTER_CLI_HOME", dir);
        }
        Self { prev }
    }
}
impl Drop for ApexRouterHomeGuard {
    fn drop(&mut self) {
        unsafe {
            match &self.prev {
...
/// `$APEXROUTER_CLI_HOME/skills/auto/<name>/SKILL.md`.
...
#[tokio::test]
#[serial(apexrouter_home_env)]
async fn auto_drafted_skill_is_loaded_next_session() {
    let home = TempDir::new().unwrap();
    let _guard = ApexRouterHomeGuard::set(home.path());
    write_auto_skill(home.path(), "auto-code-refactor");

    // A non-tempdir cwd with no project skills keeps the discovery focused on
    // the `$APEXROUTER_CLI_HOME` read path. `bare = false` so user-tier dirs (incl.
    // the new `apexrouter_home_skills_dirs`) are consulted.
    ...
    assert!(
        hit.is_some(),
        "auto-drafted skill under $APEXROUTER_CLI_HOME/skills/auto/ was not loaded; got: {:?}",
