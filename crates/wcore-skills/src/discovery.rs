...
/// Manages runtime discovery of `.apexrouter-cli/skills/` directories found in
...
impl RuntimeDiscovery {
    ...
    /// Discover `.apexrouter-cli/skills/` directories by walking up from each file path to `cwd`.
    ///
    ...
    pub async fn discover_dirs_for_paths(
        ...
        for &file_path in file_paths {
            ...
            loop {
                ...
                let skill_dir = current.join(".apexrouter-cli").join("skills");
    ...
    /// already known not to contain a `.apexrouter-cli/skills/` subdirectory.
    ...
    /// `.apexrouter-cli/skills/` subdirectories.
    ///
