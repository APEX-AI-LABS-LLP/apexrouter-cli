...
/// Helper for format_skills_within_budget tests: a minimal `SkillRef` builder
/// mirroring `make_skill`'s defaults. Returned `SkillRef`s pass through the
/// listing path that no longer requires bodies.
fn make_ref(name: &str) -> SkillRef {
    SkillRef {
        name: name.to_string(),
        display_name: None,
...
// AC-5: $ARGUMENTS, $0, ${WCORE_SKILL_DIR} are correctly substituted
// ---------------------------------------------------------------------------
...
async fn tc_e2e_2_inline_variable_substitution() {
    ...
    assert!(
        ...
        "${{WCORE_SKILL_DIR}} should be replaced with skill root, got: {result}"
    );
    ...
    assert!(
        !result.contains("${WCORE_SKILL_DIR}"),
        "${{WCORE_SKILL_DIR}} literal should not remain, got: {result}"
    );
}
...
fn tc_e2e_7_bundled_skills_budget_protection() {
    let mut bundled = make_ref("bundled-skill");
    ...
    let regular_skills: Vec<SkillRef> = (0..20)
        .map(|i| {
            let mut s = make_ref(&format!("regular-skill-{i}"));
...
// AC-14: legacy command files from .apexrouter-cli/commands/ are loaded as SkillDefinition
// ---------------------------------------------------------------------------
...
async fn tc_e2e_11_legacy_commands_loaded() {
    ...
    // load_all_skills looks for .apexrouter-cli/commands/ relative to cwd.
    let commands_dir = tmp.path().join(".apexrouter-cli").join("commands");
    ...
    let skill = skills
        ...
        .expect("legacy-cmd should be loaded from flat .md file in .apexrouter-cli/commands/");
...
// ===========================================================================
// White-box tests [white-box] - Phase 14 stage 5b
//
...
// ---------------------------------------------------------------------------
// WB-1: frontmatter - extract_frontmatter_bounds branches [white-box]
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// WB-2: permissions - auto_approve branch [white-box]
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// WB-3: conditional - activated_names persists across clear_dormant [white-box]
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// WB-4: substitution - argument parsing edge cases [white-box]
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// WB-5: shell - format_output edge cases [white-box]
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// WB-6: loader - build_namespace colon separation [white-box]
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// WB-7: context_modifier - is_empty and from_skill branches [white-box]
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// WB-8: prompt - format_skills_within_budget edge cases [white-box]
// ---------------------------------------------------------------------------
...
fn wb_8b_single_skill_within_budget() {
    let mut skill = make_ref("my-skill");
