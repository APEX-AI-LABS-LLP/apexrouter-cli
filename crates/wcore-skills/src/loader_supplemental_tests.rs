...
async fn tc_11_1_bare_mode_only_loads_add_dirs() {
    ...
    // Put a skill in add_dir's .apexrouter-cli/skills/
    let add_skills_dir = add_tmp.path().join(".apexrouter-cli").join("skills");
...
async fn tc_11_5_empty_scenario_returns_empty_vec() {
    ...
    let tmp = TempDir::new().unwrap();
    // tmp exists but has no .apexrouter-cli/skills
...
// matching TS skillRoot used for ${WCORE_SKILL_DIR} substitution.
// -----------------------------------------------------------------------
...
fn tc_wb_deduplicate_by_name_first_wins() {
    // [white-box] TC-WB: deduplicate_by_name keeps first occurrence (first-wins semantic)
...
fn tc_wb_deduplicate_by_name_empty() {
    // [white-box] empty input -> empty output
...
fn tc_wb_deduplicate_by_name_all_unique() {
    // [white-box] no duplicates - all preserved in order
    let fm = FrontmatterData::default();
...
fn tc_wb_deduplicate_by_name_case_sensitive() {
    // [white-box] name matching is case-sensitive - "Skill" and "skill" are different
    let fm = FrontmatterData::default();
    let make_meta = |name: &str| {
        crate::frontmatter::parse_skill_fields(
            &fm,
...
async fn tc_4_5_mcp_manager_none_returns_no_mcp_skills() {
    // [black-box] TC-4.5: mcp_manager=None -> no MCP skills in result
    let tmp = TempDir::new().unwrap();
    let root = tmp.path();
    fs::create_dir(root.join(".git")).unwrap();
    let skills_dir = root.join(".apexrouter-cli").join("skills");
