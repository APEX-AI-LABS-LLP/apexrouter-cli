...
pub fn substitute_arguments(
    ...
    // SAFETY: static literal regex; verified by the unit tests in this
    // module. Failure here is a checked-in-source bug, never a runtime
    // condition.
    ...
    // SAFETY: static literal regex; verified by the unit tests.
...
mod tests {
    ...
    #[test]
    fn test_skill_dir_substitution() {
        let result =
            substitute_arguments("dir=${WCORE_SKILL_DIR}", None, &[], Some("/my/skill"), None);
        assert_eq!(result, "dir=/my/skill");
    }

    #[test]
    fn test_session_id_substitution() {
        let result =
            substitute_arguments("sid=${WCORE_SESSION_ID}", None, &[], None, Some("sess-123"));
...
mod supplemental_tests {
    ...
    fn tc_6_3_skill_dir_multiple_occurrences() {
        let r = substitute_arguments(
            "${WCORE_SKILL_DIR}/a and ${WCORE_SKILL_DIR}/b",
            None,
            &[],
            Some("/skills/foo"),
            None,
    ...
    fn tc_mixed_tokens_in_one_string_both_expand() {
        ...
        assert_eq!(r, "/skills/foo/run sid-1");
    }
    ...
    fn tc_9_1_multiple_placeholder_types() {
        let r = substitute_arguments(
            "cd ${WCORE_SKILL_DIR} && run $ARGUMENTS[0] with $ARGUMENTS",
            Some("alpha beta"),
            &[],
            Some("/skills/foo"),
            None,
