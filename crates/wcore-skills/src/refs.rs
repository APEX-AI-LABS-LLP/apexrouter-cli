...
pub struct SkillCatalog {
    ...
    /// across sibling projects' `.apexrouter-cli/skills/` directories before
...
impl SkillCatalog {
    ...
    /// `.apexrouter-cli/skills/` directory is searched for the named skill.
    ///
    ...
    /// `.apexrouter-cli/skills/` directory for a skill named `name`. The first
    ...
    async fn resolve_cross_project(&self, name: &str) -> Option<SkillMetadata> {
        ...
        for project in projects {
            ...
            // `<project_dir>/.apexrouter-cli/skills/`.
            let Some(project_dir) = project.memory_db_path.parent() else {
                continue;
            };
            let skills_dir = project_dir.join(".apexrouter-cli").join("skills");
...
mod tests {
    ...
    /// plus a `.apexrouter-cli/skills/<skill>/SKILL.md` body.
    fn make_sibling_project(root: &std::path::Path, project: &str, skill: &str) {
        ...
        let skill_dir = proj.join(".apexrouter-cli").join("skills").join(skill);
        std::fs::create_dir_all(&skill_dir).unwrap();
        std::fs::write(
