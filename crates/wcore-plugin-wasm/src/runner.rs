...
use crate::host_adapters::http::{
    DenyHostHttp, EmptySecretSource, GatedHostHttp, SecretSource, ApexRouterHostHttp,
};
use crate::host_adapters::log::{GatedHostLog, ApexRouterHostLog};
use crate::host_adapters::secrets::{DenyHostSecrets, GatedHostSecrets, ApexRouterHostSecrets};
use crate::host_adapters::tools::{DenyHostTools, GatedHostTools, ToolRegistry, ApexRouterHostTools};
use crate::host_adapters::workspace::{
    DenyHostWorkspace, GatedHostWorkspace, ApexRouterHostWorkspace,
};
...
pub struct HostState {
    /// HTTP egress adapter.
    pub http: Box<dyn ApexRouterHostHttp>,
    /// Workspace filesystem adapter.
    pub workspace: Arc<dyn ApexRouterHostWorkspace>,
    /// Secret-existence adapter (existence-only by construction).
    pub secrets: Arc<dyn ApexRouterHostSecrets>,
    /// Tool-invocation adapter.
    pub tools: Arc<dyn ApexRouterHostTools>,
    /// Log adapter (always allowed; no `Deny*` variant).
    pub log: Arc<dyn ApexRouterHostLog>,
...
impl LoadedWasmPlugin {
    ...
    pub fn build_host_state(&self) -> HostState {
        ...
        // ---- HTTP ----------------------------------------------------
        let http: Box<dyn ApexRouterHostHttp> = if self.permissions.allow_network {
        ...
        let workspace: Arc<dyn ApexRouterHostWorkspace> =
        ...
        // ---- Secrets -------------------------------------------------
        let secrets: Arc<dyn ApexRouterHostSecrets> = if self.permissions.permitted_secrets.is_empty()
        {
        ...
        // ---- Tool-invoke ---------------------------------------------
        let tools: Arc<dyn ApexRouterHostTools> = if self.permissions.allow_tool_invoke {
        ...
        // ---- Log -----------------------------------------------------
        let log: Arc<dyn ApexRouterHostLog> = Arc::new(GatedHostLog::new(plugin));
    ...
    ///    `ApexrouterTool::add_to_linker` (delegates each WIT method to the
    ...
    pub async fn call_tool(
        ...
        // Linker: register every host import for the `apexrouter-tool` world.
        let mut linker: Linker<HostState> = Linker::new(&self.engine);
        crate::bindings::tool::ApexrouterTool::add_to_linker::<
            _,
            wasmtime::component::HasSelf<HostState>,
        >(&mut linker, |s: &mut HostState| s)
        .map_err(|e| WasmPluginError::ExecuteFailed(e.context("add_to_linker")))?;

        let bindings = crate::bindings::tool::ApexrouterTool::instantiate_async(
            &mut store,
            &self.component,
            &linker,
        )
        ...
        let request = crate::bindings::tool::exports::apexrouter::host::tool::Request {
...
/// `$TMPDIR/apexrouter/plugin-workspace` — no inter-plugin isolation and a
/// predictable world-reachable staging path that combined with the symlink
/// hole (M-5) to escape the sandbox.
///
/// Now we derive `<base>/apexrouter/plugin-workspace/<sanitized-name>-<hash>`,
...
fn component_workspace_root(manifest: &PluginManifest) -> PathBuf {
    ...
    let root = base.join("apexrouter").join("plugin-workspace").join(leaf);
...
/// down the apexrouter-owned subtree.
