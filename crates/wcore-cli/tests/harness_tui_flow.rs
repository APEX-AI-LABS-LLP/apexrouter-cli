...
//! `apexrouter-cli` binary attached to a PTY, send keystrokes the way a
...
//!
//! Hermetic environment: every test points `APEXROUTER_CLI_HOME` at a tempdir
//! (F-010 hermetic-sandbox env in `wcore-config::apexrouter_config_dir()`)
...
/// `Workspace`. The spawn helper sets `APEXROUTER_CLI_HOME=<home>` (F-010), so
/// `wcore_config::apexrouter_config_dir()` returns `<home>` directly —
...
/// Drives one PTY-attached `apexrouter-cli` process for a single test.
///
...
impl PtyHarness {
    /// Spawn `apexrouter-cli` on a fresh PTY sized 120x40.
    ///
    ...
    /// the same `APEXROUTER_CLI_HOME` to assert the prior conversation is restored.
    fn spawn_with_args(home: &Path, args: &[&str]) -> Self {
        ...
        // ambient provider key. Cwd is the tempdir so any `.apexrouter-cli.toml`
        ...
        // `apexrouter_config_dir()` honours this before falling back to
        // platform-native paths, so the seeded `<home>/config.toml`
        // resolves cleanly on Windows (where `HOME` alone would leak
        // to `%APPDATA%\apexrouter-cli\`).
        cmd.env("APEXROUTER_CLI_HOME", home);
...
/// Boot the TUI and block until `APEXROUTER` (the chrome wordmark) lands on
...
fn slash_exit_via_palette_terminates_the_session_cleanly() {
    ...
    let status = h
        ...
        .expect("apexrouter-cli did not exit within 8s of /exit");
    assert!(
        status.success(),
...
fn agent_turn_streams_mock_assistant_text_into_the_transcript() {
    ...
    let server = rt.block_on(
        support::mock_llm::MockLlm::new()
            .text("APEXROUTER_CLI_MOCK_STREAM_OK the agent loop is wired end to end")
            .start(),
    );
    ...
    h.wait_for(
        |s| s.contains("APEXROUTER_CLI_MOCK_STREAM_OK"),
...
fn tool_call_renders_approval_then_executes_and_continues_on_approve() {
    ...
    let server = rt.block_on(
        support::mock_llm::MockLlm::new()
            ...
            .text("APEXROUTER_CLI_TOOL_DONE the write was approved and the turn continued")
            .start(),
    );
    ...
    h.wait_for(
        |s| s.contains("APEXROUTER_CLI_TOOL_DONE"),
        Duration::from_secs(30),
...
fn provider_transient_error_retries_then_streams_the_response() {
    ...
    let server = rt.block_on(
        support::mock_llm::MockLlm::new()
            .http_error(503)
            .text("APEXROUTER_CLI_RETRY_OK recovered after a transient 503")
            .start(),
    );
    ...
    h.wait_for(
        |s| s.contains("APEXROUTER_CLI_RETRY_OK"),
        Duration::from_secs(30),
...
/// turn in one process is persisted to `$APEXROUTER_CLI_HOME/sessions/`, the process
...
fn session_save_resume_threads_prior_history_into_the_next_request() {
    ...
    // $APEXROUTER_CLI_HOME/sessions/<date>_<id>.json (index.json is the catalog).
...
fn esc_cancels_an_in_flight_turn_and_the_session_keeps_working() {
    ...
    let server = rt.block_on(
        support::mock_llm::MockLlm::new()
            .slow_text("APEXROUTER_CLI_SHOULD_NOT_APPEAR slow turn body", 4000)
            .text("APEXROUTER_CLI_RECOVERED_OK the session still works after cancel")
            .start(),
    );
    seed_config_with_base_url(home.path(), &server.uri());
    let mut h = boot_to_workspace(home.path());
    ...
    assert!(
        !after_cancel.contains("APEXROUTER_CLI_SHOULD_NOT_APPEAR"),
    ...
    h.send(b"\r");
    h.wait_for(
        |s| s.contains("APEXROUTER_CLI_RECOVERED_OK"),
        Duration::from_secs(30),
