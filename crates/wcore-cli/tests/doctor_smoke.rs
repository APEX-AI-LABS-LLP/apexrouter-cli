//! W5 (A.5): CLI integration smoke test for `apexrouter-cli --doctor`.
//!
...
/// Run the compiled `apexrouter-cli` binary with `--doctor` and return
/// the captured output. The harness sets `CARGO_BIN_EXE_apexrouter-cli`
...
fn run_doctor() -> std::process::Output {
    ...
    Command::new(bin)
        .arg("--doctor")
        .output()
        .expect("spawn apexrouter-cli --doctor")
}
...
fn doctor_emits_header_and_summary() {
    ...
    assert!(
        stdout.contains("apexrouter-cli doctor v"),
