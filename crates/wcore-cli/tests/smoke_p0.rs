...
//!
//! 1. **Engine-behavior checks** — provable by driving the REAL `apexrouter-cli`
...
//!
//! Every check points `APEXROUTER_CLI_HOME` (and `HOME`) at a throwaway tempdir (the
//! F-010 hermetic-sandbox override honoured by
//! `wcore_config::apexrouter_config_dir()`) and strips the full provider-credential
...
/// Apply the hermetic child env uniformly: point `APEXROUTER_CLI_HOME` + `HOME` at the
...
/// TRAILING POSITIONAL argument — `apexrouter-cli` has no `-p` flag (`-p` is the
...
/// provider-key set (see [`STRIPPED_PROVIDER_ENV`]), sets `APEXROUTER_CLI_HOME` +
...
fn run_headless(home: &Path, args: &[&str]) -> (std::process::ExitStatus, String, String) {
    ...
    let out = cmd.output().expect("spawn apexrouter-cli headless");
    (
        out.status,
...
mod pty {
    ...
    fn gap_d015_double_slash_does_not_poison_or_brick() {
        ...
        assert!(
            screen.contains("APEXROUTER") && screen.contains("Workspace"),
    ...
    /// banner whose only remedy is "run `apexrouter-cli setup`" or hand-edit
    ...
    /// GAP D011 (project layer) — a corrupt PROJECT `.apexrouter-cli.toml` under an
    ...
    /// non-zero with anyhow's `Error: failed to parse .apexrouter-cli.toml: ...`
    ...
    fn gap_d011_corrupt_project_config_under_tui_surfaces_parse_error() {
        ...
        // parse-class wording and the `.apexrouter-cli.toml` filename together, so a
        ...
        let parse_error_names_project_file = screen.lines().any(|line| {
            ...
            l.contains(".apexrouter-cli.toml")
        ...
        assert!(
            ...
            "corrupt PROJECT config under a TUI launch must surface a visible parse error that \
             NAMES .apexrouter-cli.toml and refuse the silent onboarding downgrade (non-zero exit); \
             got exit success={}\n--- terminal ---\n{screen}\n--- end ---",
            status.success()
        );
    }
...
fn live_real_key_first_prompt_round_trip() {
    ...
    cmd.args(["--no-tui", "Reply with exactly: SMOKE_LIVE_OK"])
        .current_dir(home.path())
        .env("APEXROUTER_CLI_HOME", home.path())
        .env("HOME", home.path())
        .env("TERM", "dumb")
        .stdin(std::process::Stdio::null())
        .stdout(std::process::Stdio::piped())
        .stderr(std::process::Stdio::piped());
    let out = cmd.output().expect("spawn live apexrouter-cli");
