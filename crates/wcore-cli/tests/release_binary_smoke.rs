...
//! exposes through `env!("CARGO_BIN_EXE_apexrouter-cli")`. That is
...
//!
//! - `capabilities.browser_suite == true` (apexrouter-browser linked)
//! - `capabilities.computer_use  == true` (apexrouter-cua linked — flag is
...
//!
//! Any future regression that drops a `use apexrouter_<plugin> as _;` from
...
/// release artifact at `target/release/apexrouter-cli` instead of the
/// debug binary Cargo wires through `CARGO_BIN_EXE_apexrouter-cli`.
fn first_ready_event_release() -> serde_json::Value {
    let bin = ensure_release_binary();

    // Clean cwd + HOME so no `.apexrouter-cli.toml` from the dev environment
    ...
    let mut child = Command::new(&bin)
        ...
        .stderr(Stdio::piped())
        .spawn()
        .expect("spawn release apexrouter-cli --json-stream");
...
fn release_binary_ready_event_advertises_plugin_capabilities() {
    ...
    // apexrouter-browser plugin inventory items survived release LTO.
    assert_eq!(
        caps["browser_suite"], true,
        "release binary: expected capabilities.browser_suite=true (apexrouter-browser stripped \
         by release LTO?); caps: {caps}"
    );

    // apexrouter-cua plugin presence flips this — independent of the
    ...
    assert_eq!(
        ...
        "release binary: expected capabilities.computer_use=true (apexrouter-cua stripped by \
