...
fn install_then_list_then_remove_round_trips() {
    ...
    std::fs::write(
        registry_dir.join("apexrouter-honcho.toml"),
        r#"
name = "apexrouter-honcho"
version = "0.6.0"
...
"#,
    ...
    wcore_cli::plugin::install::install_from_registry(&store, "apexrouter-honcho", &install_root)
        .unwrap();
    let installed = wcore_cli::plugin::install::list_installed(&install_root).unwrap();
    assert_eq!(installed.len(), 1);
    assert_eq!(installed[0].name, "apexrouter-honcho");
    assert_eq!(installed[0].version, "0.6.0");

    wcore_cli::plugin::install::remove(&install_root, "apexrouter-honcho").unwrap();
...
fn remove_unknown_returns_not_installed() {
    ...
    let err = wcore_cli::plugin::install::remove(&install_root, "apexrouter-honcho").unwrap_err();
    assert!(matches!(
        err,
...
fn default_registry_loads_all_v0_6_entries() {
    ...
    assert!(names.contains(&"apexrouter-ollama"));
    assert!(names.contains(&"apexrouter-honcho"));
    assert!(names.contains(&"apexrouter-browser"));
    assert!(names.contains(&"apexrouter-cua"));
    // apexrouter-ijfw is a declarative-install-only integration (installed via
    // the IJFW skill/MCP path, not bundled into the binary). It must NOT
    // appear in the embedded install catalog.
    assert!(!names.contains(&"apexrouter-ijfw"));
}
