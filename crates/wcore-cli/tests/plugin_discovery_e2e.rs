...
//! showed that `crates/wcore-cli/Cargo.toml` listing `apexrouter-browser`,
//! `apexrouter-cua`, `apexrouter-ollama` as dependencies was
...
//! `use apexrouter_<plugin> as _;` lines at the top of `src/main.rs`.
//!
...
fn first_ready_event() -> serde_json::Value {
    // Use a clean, empty cwd so no `.apexrouter-cli.toml` from the dev
    ...
    let bin = env!("CARGO_BIN_EXE_apexrouter-cli");
    let mut child = Command::new(bin)
        ...
        .stderr(Stdio::piped())
        .spawn()
        .expect("spawn apexrouter-cli --json-stream");
...
fn ready_event_has_plugin_capability_flags() {
    ...
    // The apexrouter-browser plugin must produce a true `browser_suite` flag.
    assert_eq!(
        caps["browser_suite"], true,
        "expected capabilities.browser_suite=true (apexrouter-browser plugin not discovered); \
         caps: {caps}"
    );

    // The apexrouter-cua plugin must produce a true `computer_use` flag.
    assert_eq!(
        caps["computer_use"], true,
        "expected capabilities.computer_use=true (apexrouter-cua plugin not discovered); \
         caps: {caps}"
    );
