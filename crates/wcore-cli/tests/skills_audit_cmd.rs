//! CLI integration: `apexrouter-cli --skills-audit` against a fixture project.
...
fn fixture_project() -> TempDir {
    ...
    let dir = tmp.path().join(".apexrouter-cli").join("skills").join("ok");
...
fn skills_audit_writes_json_and_renders_markdown() {
    ...
    let json_path = tmp.path().join(".apexrouter-cli").join("skills-audit.json");
