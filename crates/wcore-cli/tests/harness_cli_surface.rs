...
//! the library level. This layer drives the **compiled `apexrouter-cli`
...
//! binary Cargo wires through `CARGO_BIN_EXE_apexrouter-cli`, so it runs in
...
//! the real `~/Library/Application Support/apexrouter-cli/config.toml`
//! (macOS) / `~/.config/apexrouter-cli/config.toml` (Linux) is never read
...
///
/// `APEXROUTER_CLI_HOME` is the F-010 hermetic-sandbox env in `wcore-config`'s
/// `apexrouter_config_dir()` resolver — it overrides every config/auth path
/// on all three platforms in one shot. `HOME` alone is *not* enough on
/// Windows, where `dirs::config_dir()` resolves via `%APPDATA%`, not
/// `HOME` — so tests would write to the real `%APPDATA%\apexrouter-cli\`
...
fn run_isolated(args: &[&str], home: &Path) -> Output {
    Command::new(binary())
        .args(args)
        // A scratch cwd too: `apexrouter-cli` reads `.apexrouter-cli.toml`
        // from cwd, and the repo root has none — but pointing cwd at the
        // tempdir removes any doubt.
        .current_dir(home)
        .env("HOME", home)
        .env("APEXROUTER_CLI_HOME", home)
...
fn version_reports_the_pinned_release() {
    ...
    let expected = format!("apexrouter-cli {}", env!("CARGO_PKG_VERSION"));
    assert!(
...
fn help_lists_the_setup_and_auth_subcommands() {
    ...
    assert!(
        stdout.contains("Usage: apexrouter-cli"),
...
fn an_unrecognized_argument_fails_with_a_usage_message() {
    // NOTE: a bare unknown WORD (e.g. `apexrouter-cli notacommand`) is NOT
    ...
    assert!(
        stderr.contains("error:") && stderr.contains("Usage: apexrouter-cli"),
...
fn an_unrecognized_subcommand_under_auth_fails_with_usage() {
    ...
    let stderr = stderr_of(&out);
    // Windows runs the binary as `apexrouter-cli.exe`, so clap's usage
    // line reads `Usage: apexrouter-cli.exe auth ...` — the exact-substring
    // `Usage: apexrouter-cli auth` doesn't match (CI run 26405564483 job
