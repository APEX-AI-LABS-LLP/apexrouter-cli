...
//!
//! This file covers R-001..R-015 against the compiled `apexrouter-cli` binary.
...
fn seed_desktop_jobs_json(apexrouter_home: &Path) {
    let cron_dir = apexrouter_home.join("cron");
...
/// first, then target/{release,debug}/apexrouter-cli relative to workspace root.
...
fn maybe_eval_binary() -> Option<std::path::PathBuf> {
    ...
    for profile in ["release", "debug"] {
        let cand = workspace.join("target").join(profile).join("apexrouter-cli");
        if cand.exists() {
            return Some(cand);
        }
    }
...
/// `<root>/.apexrouter-cli/config.toml` with absolute session dir.
...
fn seed_json_stream_env(
    ...
    let cfg_dir = root.join(".apexrouter-cli");
...
fn r001_cron_schedule_alias_deserializes() {
    ...
    let apexrouter_home = home.path().join(".apexrouter");
    std::fs::create_dir_all(&apexrouter_home).expect("create .apexrouter");

    seed_desktop_jobs_json(&apexrouter_home);

    let out = Command::new(binary())
        .args(["cron", "list"])
        .current_dir(home.path())
        .env("HOME", home.path())
        .env("APEXROUTER_CLI_HOME", &apexrouter_home)
        .env_remove("API_KEY")
...
fn r002_yaml_config_migration() {
    ...
    // Drop a legacy IJFW-style config.yaml under APEXROUTER_CLI_HOME.
    let yaml_body = "model:\n  default: gpt-4o-mini\n  provider: openai\n";
    std::fs::write(apexrouter_home.join("config.yaml"), yaml_body).expect("write legacy config.yaml");
...
async fn r003_no_model_guard() {
    let Some(bin) = maybe_eval_binary() else {
        eprintln!("[R-003 SKIP] apexrouter-cli binary not found — build first or set WCORE_EVAL_BIN");
        return;
    };
    ...
    // Seed a hermetic .apexrouter-cli/config.toml with NO model.
    let cfg_dir = home.path().join(".apexrouter-cli");
    std::fs::create_dir_all(&cfg_dir).expect("create cfg dir");
...
async fn r004_crash_sentinel_clears_on_clean_exit() {
    let Some(bin) = maybe_eval_binary() else {
        eprintln!("[R-004 SKIP] apexrouter-cli binary not found");
        return;
    };
...
async fn r005_force_mode_aliases() {
    let Some(bin) = maybe_eval_binary() else {
        eprintln!("[R-005 SKIP] apexrouter-cli binary not found");
        return;
    };
...
async fn r006_init_history_injects_system_prompt() {
    let Some(bin) = maybe_eval_binary() else {
        eprintln!("[R-006 SKIP] apexrouter-cli binary not found");
        return;
    };
...
async fn r007_session_wal_survives_mid_turn_kill() {
    let Some(bin) = maybe_eval_binary() else {
        eprintln!("[R-007 SKIP] apexrouter-cli binary not found");
        return;
    };
...
async fn r010_openrouter_url_no_double_v1() {
    ...
    let Some(bin) = maybe_eval_binary() else {
        eprintln!("[R-010 SKIP] apexrouter-cli binary not found");
        return;
    };
...
async fn r013_customer_flow_assistants() {
    ...
    let Some(bin) = maybe_eval_binary() else {
        eprintln!("[R-013 SKIP] apexrouter-cli binary not found");
        return;
    };
...
async fn r014_customer_flow_skills_hello() {
    ...
    let Some(bin) = maybe_eval_binary() else {
        eprintln!("[R-014 SKIP] apexrouter-cli binary not found");
        return;
    };
