...
fn validates_plugin_name_rejects_path_traversal() {
    ...
    assert!(validate_plugin_name("apexrouter-honcho").is_ok());
...
fn github_release_url_is_constructed_safely() {
    ...
    let r = GitHubReleasesResolver::new("APEX-AI-LABS-LLP");
    let url = r.release_api_url("apexrouter-honcho").unwrap();
    assert_eq!(
        url.as_str(),
        "https://api.github.com/repos/APEX-AI-LABS-LLP/apexrouter-honcho/releases/latest"
    );
...
fn local_file_resolver_round_trips_a_manifest() {
    ...
    std::fs::write(
        dir.join("apexrouter-honcho.toml"),
        r#"
...
"#,
    ...
    let mf = r.resolve_manifest("apexrouter-honcho").unwrap();
    assert_eq!(mf.name, "apexrouter-honcho");
    ...
    let err = r.resolve_manifest("apexrouter-wormhole").unwrap_err();
    assert!(matches!(
        err,
...
fn install_via_local_file_resolver_writes_record() {
    ...
    std::fs::write(
        registry.join("apexrouter-ollama.toml"),
        r#"
...
"#,
    ...
    install_via_resolver(&r, "apexrouter-ollama", &install_root).unwrap();
    let entries = list_installed(&install_root).unwrap();
    assert_eq!(entries.len(), 1);
    assert_eq!(entries[0].name, "apexrouter-ollama");
}
