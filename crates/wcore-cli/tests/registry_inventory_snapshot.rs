...
//!
//!   1. CUA — `apexrouter-cua` plugin loads + registrar captures the spec,
...
//!   2. Browser policy — `apexrouter-browser` registers a spec with
...
async fn bootstrap_registers_cua_tool_after_wiring_fix() {
    ...
    assert!(
        ...
        "Cua tool must be registered after bootstrap. \
         ...
         flips the registrar gate so `apexrouter-cua`'s captured spec reifies. \
         Got tools: {names:?}"
    );
}
