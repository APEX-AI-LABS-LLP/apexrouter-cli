...
//!
//! Writes a file at `$APEXROUTER_CLI_HOME/.dirty-death` on startup and removes it on
...
/// Filename of the flag written under `$APEXROUTER_CLI_HOME`.
const FLAG_FILE: &str = ".dirty-death";

/// Environment variable that overrides the default apexrouter home directory.
const APEXROUTER_CLI_HOME_ENV: &str = "APEXROUTER_CLI_HOME";

/// Subdirectory of `$HOME` used when `APEXROUTER_CLI_HOME` is unset.
...
impl CrashSentinel {
    /// Resolve the default flag path, honoring `APEXROUTER_CLI_HOME` when set, else
    /// `$HOME/.apexrouter/.dirty-death`, with a final fallback to `./.apexrouter/`
...
mod tests {
    ...
    #[test]
    #[serial(env)]
    fn default_path_honors_apexrouter_home_env() {
        let dir = TempDir::new().unwrap();
        ...
        unsafe {
            std::env::set_var(APEXROUTER_CLI_HOME_ENV, dir.path());
        }
        let resolved = CrashSentinel::default_path();
        unsafe {
            std::env::remove_var(APEXROUTER_CLI_HOME_ENV);
        }
