//! `apexrouter-cli acp` — production caller for the ACP crate. Two
...
/// Top-level args for `apexrouter-cli acp ...`.
#[derive(Args, Debug)]
pub struct AcpArgs {
    #[command(subcommand)]
    pub command: AcpCmd,
...
async fn serve(args: AcpServeArgs) -> anyhow::Result<()> {
    ...
    let api_key = match wcore_config::keychain::get_secret(
        ...
        _ => {
            ...
            eprintln!(
                "apexrouter-cli acp: generated API key (first run) — \
    ...
    let agent_id = hostname_or("apexrouter-cli");
    ...
    eprintln!(
        "apexrouter-cli acp: serving on http://{local} \
