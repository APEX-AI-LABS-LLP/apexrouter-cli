...
//! the `apexrouter-cli auth` subcommand (`crate::auth`) import from here so
...
/// A provider ApexRouter can connect to with an API key.
///
...
/// (`apexrouter-cli auth`). Driving a nested `Runtime` from the latter would
