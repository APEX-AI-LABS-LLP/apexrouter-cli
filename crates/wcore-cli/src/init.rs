//! `apexrouter-cli init` — scaffold a `.apexrouter/` directory in the
//! current project with a starter `config.toml` plus a `APEXROUTER.md`
//! at the project root.
//!
//! The `APEXROUTER.md` ships the portable §0-9 agent discipline (the
...
//!
//! v0.7.0 1.B.2: non-interactive by design. Reads `APEXROUTER_CLI_MODEL`
...
/// `APEXROUTER_CLI_MODEL` nor `--model` is supplied. Kept here as a constant
...
const APEXROUTER_CLI_MD_TEMPLATE: &str = include_str!("init_template.md");
...
pub struct InitArgs {
    ...
    /// omitted, falls back to `$APEXROUTER_CLI_MODEL` and then to
    /// [`PLACEHOLDER_MODEL`].
    #[arg(long)]
    pub model: Option<String>,

    /// Skip the `APEXROUTER.md` template at the project root. The
    /// `.apexrouter/config.toml` is still written.
    #[arg(long)]
...
pub struct InitOutcome {
    /// `.apexrouter/config.toml` was written this run.
    pub wrote_config: bool,
    /// `APEXROUTER.md` was written this run.
...
pub fn render_config_toml(model: &str) -> String {
    format!(
        "# ApexRouter-CLI project config.\n\
         # v0.7.0 1.B.2: generated by `apexrouter-cli init`. Edit freely;\n\
         ...
         # Disable in CI by setting APEXROUTER_CLI_MEMORY=0.\n\
         enabled = true\n\
         \n\
         [observability]\n\
...
/// Render the full `APEXROUTER.md`: the portable §0-9 discipline scaffold
...
pub fn render_section_10(project_name: &str, scan: &ProjectScan) -> String {
    ...
    let mut body = format!(
        "**Stack:** {} (detected by `apexrouter-cli init`). _Refine this: describe what \
...
pub fn run(args: InitArgs) -> std::io::Result<InitOutcome> {
    ...
    let model = args
        .model
        .clone()
        .or_else(|| std::env::var("APEXROUTER_CLI_MODEL").ok())
        .unwrap_or_else(|| PLACEHOLDER_MODEL.to_string());

    let mut outcome = InitOutcome::default();

    let apexrouter_dir = root.join(".apexrouter");
    fs::create_dir_all(&apexrouter_dir)?;

    let config_path = apexrouter_dir.join("config.toml");
    if config_path.exists() && !args.force {
        outcome.skipped.push(".apexrouter/config.toml".to_string());
    } else {
    ...
    if !args.skip_context {
        let context_path = root.join("APEXROUTER.md");
        if context_path.exists() && !args.force {
            outcome.skipped.push("APEXROUTER.md".to_string());
        } else {
...
pub fn print_summary(outcome: &InitOutcome) {
    if outcome.wrote_config {
        eprintln!("apexrouter-cli: wrote .apexrouter/config.toml");
    }
    if outcome.wrote_context {
        eprintln!("apexrouter-cli: wrote APEXROUTER.md");
    }
    for path in &outcome.skipped {
        eprintln!("apexrouter-cli: skipped {path} (already exists; pass --force to overwrite)");
    }
    if !outcome.wrote_config && !outcome.wrote_context && outcome.skipped.is_empty() {
        eprintln!("apexrouter-cli: nothing to do (--skip-context and config exists)");
    }
}
...
mod tests {
    ...
    fn fresh_dir_writes_both_files() {
        ...
        assert!(tmp.path().join(".apexrouter/config.toml").exists());
        assert!(tmp.path().join("APEXROUTER.md").exists());
    }
    ...
    fn force_overwrites_existing() {
        let tmp = TempDir::new().unwrap();
        std::fs::create_dir_all(tmp.path().join(".apexrouter")).unwrap();
        std::fs::write(tmp.path().join(".apexrouter/config.toml"), "old").unwrap();
        std::fs::write(tmp.path().join("APEXROUTER.md"), "old").unwrap();
        ...
        let body = std::fs::read_to_string(tmp.path().join(".apexrouter/config.toml")).unwrap();
        assert_ne!(body, "old");
    ...
    fn placeholder_used_when_no_model_anywhere() {
        // Save + unset APEXROUTER_CLI_MODEL for this test.
        let saved = std::env::var("APEXROUTER_CLI_MODEL").ok();
        // SAFETY: tests run single-threaded by default for this assertion.
        unsafe {
            std::env::remove_var("APEXROUTER_CLI_MODEL");
        }
        let tmp = TempDir::new().unwrap();
        ...
        if let Some(v) = saved {
            // SAFETY: same.
            unsafe {
                std::env::set_var("APEXROUTER_CLI_MODEL", v);
            }
        }
    }

    #[test]
    fn deployed_apexrouter_md_ships_the_discipline_scaffold() {
        let tmp = TempDir::new().unwrap();
        run(args_in(tmp.path())).unwrap();
        let md = std::fs::read_to_string(tmp.path().join("APEXROUTER.md")).unwrap();
        ...
        assert!(
            ...
            "unsubstituted placeholder leaked into APEXROUTER.md"
        );
    }
