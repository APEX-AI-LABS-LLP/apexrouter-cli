//! B2 — `apexrouter workflow <subcommand>` CLI surface for the dynamic-workflow
//! engine.
//!
...
//! - `list` — discover `.apexrouter/workflows/*.ron` under the resolved project
...
//! - `run <NAME>` — resolve `<NAME>` to `.apexrouter/workflows/<NAME>.ron`, parse,
...
//!
//! Saved workflows live under `<project-root>/.apexrouter/workflows/`. The project
//! root is found by [`find_workflows_dir`], which walks up from the cwd looking
//! for an existing `.apexrouter` directory (the same project-local convention
//! `bootstrap.rs` uses for `.apexrouter/user-model.json`). When no ancestor has a
//! `.apexrouter` dir, it falls back to `<cwd>/.apexrouter/workflows` so a fresh
...
/// `apexrouter workflow <subcommand>`.
...
pub enum WorkflowCmd {
    ...
    /// List the saved workflows discovered under `.apexrouter/workflows/*.ron`.
    ...
    /// `.apexrouter/workflows/<NAME>.ron`, parses it, and runs it through the
    /// workflow engine over a real provider.
    Run {
        /// Saved-workflow name (the `.ron` stem under `.apexrouter/workflows/`).
        name: String,
    },
}
...
async fn run_workflow(name: &str) -> anyhow::Result<()> {
    ...
    // `apexrouter-cli` launch with no provider/model flags.
...
/// file/env layers exactly as a flagless `apexrouter-cli` launch would.
...
/// Resolve `<project-root>/.apexrouter/workflows`.
///
/// Walks up from the cwd looking for an ancestor that already contains a
/// `.apexrouter` directory (the project-local convention used elsewhere, e.g.
/// `bootstrap.rs`'s `.apexrouter/user-model.json`). Falls back to
/// `<cwd>/.apexrouter/workflows` when no ancestor has one.
fn find_workflows_dir() -> anyhow::Result<PathBuf> {
    let cwd = std::env::current_dir()?;
    let root = find_project_root(&cwd).unwrap_or_else(|| cwd.clone());
    Ok(root.join(".apexrouter").join("workflows"))
}

/// Walk up from `start` returning the first ancestor (inclusive) that holds a
/// `.apexrouter` directory, or `None` if none do.
fn find_project_root(start: &Path) -> Option<PathBuf> {
    let mut cursor: Option<&Path> = Some(start);
    while let Some(dir) = cursor {
        if dir.join(".apexrouter").is_dir() {
...
"#;
    ...
    fn read_ron_files_lists_two_good_warns_on_bad() {
        // Build a temp `.apexrouter/workflows` with 2 good + 1 bad `.ron`, plus a
        // non-ron file that must be ignored entirely.
        let dir = tempfile::tempdir().unwrap();
        let wf = dir.path().join(".apexrouter").join("workflows");
    ...
    fn read_ron_files_missing_dir_is_not_found() {
        let dir = tempfile::tempdir().unwrap();
        let missing = dir.path().join(".apexrouter").join("workflows");
        let err = read_ron_files(&missing).expect_err("missing dir errors");
        assert_eq!(err.kind(), std::io::ErrorKind::NotFound);
    }

    #[test]
    fn find_project_root_walks_up_to_apexrouter_dir() {
        let dir = tempfile::tempdir().unwrap();
        let root = dir.path();
        std::fs::create_dir_all(root.join(".apexrouter")).unwrap();
        let nested = root.join("a").join("b");
        std::fs::create_dir_all(&nested).unwrap();
        // From a nested cwd, the root with `.apexrouter` is discovered.
    ...
    fn find_project_root_none_when_absent() {
        let dir = tempfile::tempdir().unwrap();
        // No `.apexrouter` anywhere under this isolated temp tree's leaf — but an
        ...
        // the leaf has no `.apexrouter` and find_project_root does not invent one.
        let leaf = dir.path().join("x");
        std::fs::create_dir_all(&leaf).unwrap();
        // The temp dir has no `.apexrouter`; walking up may still hit a real
        // ancestor, so we only assert the immediate dir is not falsely matched.
        assert!(!leaf.join(".apexrouter").is_dir());
    }
}
