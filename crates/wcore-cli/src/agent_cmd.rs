//! v0.7.0 Task 3.B.2: `apexrouter-cli agent` subcommands.
//!
...
pub enum AgentCmd {
    /// Create a new user agent (persists to ~/.apexrouter/agents/<name>.toml).
    Create {
...
pub fn run(cmd: AgentCmd) -> Result<()> {
    let base = factory::user_agent_dir().context("resolving ~/.apexrouter/agents")?;
