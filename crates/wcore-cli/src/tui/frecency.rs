...
/// `APEXROUTER_CLI_HOME` hermetically sandboxes the frecency file alongside the
/// rest of the engine's on-disk state (F-010, #270). Returns `Some` in all
/// cases since the canonical helper has a `PathBuf::from("apexrouter-cli")`
...
fn store_path() -> Option<PathBuf> {
    Some(wcore_config::config::apexrouter_config_dir().join("frecency.json"))
}
...
mod tests {
    ...
    fn save_then_load_round_trips_the_store() {
        ...
        // SAFETY: single-threaded test; APEXROUTER_CLI_HOME is the canonical
        // hermetic override resolved by `apexrouter_config_dir()` (F-010,
