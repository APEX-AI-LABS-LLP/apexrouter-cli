...
use crate::tui::widgets::{panel, render_sources, render_streaming_status, apexrouter_banner};
...
impl WorkspaceSurface {
    ...
    fn render_composer(&self, frame: &mut Frame, area: Rect, app: &App, theme: &Theme) {
        ...
        if app.config.model.is_empty() {
            ...
            // dead-end. `apexrouter-cli setup` stays as a secondary option.
            ...
            lines.push(Line::from(vec![Span::styled(
                "Or run  apexrouter-cli setup  to change provider.",
...
/// Draw the idle hero — the full APEXROUTER banner.
///
/// An empty transcript was previously blank space with a one-line prompt;
/// the hybrid-branding decision makes it the home for the full APEXROUTER
/// ASCII banner (the same wordmark the onboarding intro shows). The
/// [`apexrouter_banner`] widget paints the wordmark, the "the autonomous AI
...
/// The idle/empty Workspace hero: the APEXROUTER banner plus a factual one-line
...
/// ApexRouter is, then three copy-pasteable starter prompts. The banner keeps the
...
fn render_idle_hero(frame: &mut Frame, area: Rect, theme: &Theme) {
    ...
    if !show_hero {
        apexrouter_banner(frame, area, theme);
        return;
    }

    let [banner_area, hero_area] =
        Layout::vertical([Constraint::Min(1), Constraint::Length(HERO_ROWS)]).areas(area);
    apexrouter_banner(frame, banner_area, theme);

    // One factual line describing what ApexRouter is, then concrete starter
...
fn build_live_tail_lines(lines: &mut Vec<Line<'static>>, app: &App, theme: &Theme, stream: &str) {
    ...
    // streamed text. v0.9.1 W1 A drops the explicit `apexrouter` role
...
///
/// v0.9.1 W1 A: Assistant turns no longer render a `"apexrouter"` role
...
/// widget in the composer already owns the "apexrouter is working" affordance
...
mod tests {
    ...
    #[test]
    fn idle_state_renders_the_apexrouter_banner() {
        // An empty session is the idle state (mockup surface 02). The
        // hybrid-branding decision makes it the home for the full APEXROUTER
    ...
    fn idle_hero_shows_subtitle_and_example_prompts_d044() {
        ...
        // factual description of what ApexRouter is, plus 2-3 concrete starter
    ...
    fn completed_conversation_renders_the_assistant_turn() {
        ...
        let mut surface = WorkspaceSurface::new();
        let out = render_to_string(&mut surface, &app, 100, 30);
        // v0.9.1 W1 A: the literal `"apexrouter"` role label is GONE — the
        // 2-space gutter is the only role signal. We assert the body
        // text lands (markdown-rendered, indented). The lowercase
        // `"apexrouter"` string used to mark each assistant turn header;
        ...
        assert!(
            !out.contains("apexrouter"),
            "v0.9.1 W1 A removed the lowercase 'apexrouter' role label, but it leaked:\n{out}"
        );
    }
