...
impl PluginsSurface {
    ...
    /// default install root (`dirs::data_dir()/apexrouter-cli/plugins`).
    ...
    fn render_footer(&self, frame: &mut Frame, area: Rect, theme: &Theme) {
        ...
        let reversible = Line::from(Span::styled(
            " installs to ~/…/apexrouter-cli/plugins · you can remove any plugin anytime",
...
/// The default plugin install root: `dirs::data_dir()/apexrouter-cli/plugins`.
/// Mirrors the path logic in `crate::plugin::run` so the TUI install/remove
/// verbs target the same directory the `apexrouter-cli plugin` subcommand does.
pub(crate) fn plugin_install_root() -> anyhow::Result<PathBuf> {
    ...
    Ok(base.join("apexrouter-cli").join("plugins"))
}
...
mod tests {
    ...
    fn fixture_surface() -> PluginsSurface {
        ...
        let available = PluginRow {
            manifest: PluginManifest {
                name: "apexrouter-cua".into(),
                version: "0.6.1".into(),
                requires_sandbox: true,
                description: "computer use".into(),
                dependencies: vec!["apexrouter-browser".into()],
            },
            section: Section::Available,
        };
        PluginsSurface {
    ...
    fn render_shows_both_sections_and_their_rows() {
        ...
        assert!(out.contains("apexrouter-ollama"), "missing installed row");
        assert!(out.contains("apexrouter-cua"), "missing available row");
    ...
    fn enter_on_an_installed_row_emits_a_remove_command() {
        ...
        match action {
            SurfaceAction::Command(cmd) => {
                assert_eq!(cmd, "/plugins remove apexrouter-ollama");
            }
    ...
    fn enter_on_an_available_row_emits_an_install_command() {
        ...
        match action {
            SurfaceAction::Command(cmd) => {
                assert_eq!(cmd, "/plugins install apexrouter-cua");
            }
            _ => panic!("expected a Command action, got something else"),
        }
    }
    ...
    fn details_view_shows_dependencies_and_sandbox_flag() {
        ...
        surface.handle_key(key(KeyCode::Char('i')), &mut app);
        let out = render_to_string(&mut surface);
        assert!(out.contains("apexrouter-browser"), "dependency must show");
