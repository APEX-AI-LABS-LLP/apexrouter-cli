...
//!
//! The hybrid-branding decision: the full APEXROUTER ASCII banner is the
...
//! stats strip + a tab row) into ONE row: the `◆ APEXROUTER` wordmark on the
...
/// Contents, left to right: the `◆ APEXROUTER` wordmark, then the surface
...
pub fn top_chrome(f: &mut Frame, area: Rect, t: &Theme, selected: usize) {
    ...
    // Budget: wordmark uses ~16 cols ("  ◆ APEXROUTER   ") + 3-col gutters
...
mod tests {
    ...
    fn top_chrome_shows_the_wordmark_and_every_tab() {
        ...
        assert!(line.contains("APEXROUTER"), "wordmark missing: {line:?}");
