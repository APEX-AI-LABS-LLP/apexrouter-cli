        ...
        None => {
            return Err(
                "Google Meet auth needs `GOOGLE_CLIENT_ID` (and `GOOGLE_CLIENT_SECRET`) in \
                    `~/.apexrouter/.env`. Open `/config` → Tools & Providers → Google Meet to set \
...
async fn run_google_meet_connect(flow: OAuthFlow) -> String {
    ...
    // 6. Persist the bundle to `~/.apexrouter/oauth/google_meet.json`.
...
mod tests {
    ...
    fn google_meet_without_client_id_explains_setup() {
        ...
        assert!(out.contains(".apexrouter/.env"), "want setup hint: {out}");
        if let Some(v) = saved {
