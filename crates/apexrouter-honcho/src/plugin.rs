//! v0.6.4 Task 2.3 — Plugin / PluginFactory glue for `apexrouter-honcho`.
//!
//! `ApexRouterHonchoFactory` is submitted via `inventory::submit!` so it's
//! discoverable through the host-side `PluginLoader::discover` path without
//! any explicit registration in `main()`. `ApexRouterHoncho::initialize`
...
fn manifest() -> &'static PluginManifest {
    ...
    M.get_or_init(|| {
        ...
        PluginManifest::from_toml_str(MANIFEST_TOML)
            .expect("apexrouter-honcho plugin.toml must parse and validate")
    })
}

pub struct ApexRouterHoncho;

#[async_trait]
impl Plugin for ApexRouterHoncho {
...
pub struct ApexRouterHonchoFactory;

impl PluginFactory for ApexRouterHonchoFactory {
    fn name(&self) -> &'static str {
        "apexrouter-honcho"
    }

    fn build(&self) -> Box<dyn Plugin> {
        Box::new(ApexRouterHoncho)
    }
}

inventory::submit! { &ApexRouterHonchoFactory as &dyn PluginFactory }
