//! v0.6.4 Task 2.3 — end-to-end proof that `apexrouter-honcho`'s `Plugin`
...
use apexrouter_honcho::{ApexRouterHoncho, ApexRouterHonchoFactory};
...
fn factory_name_matches_manifest() {
    let factory = ApexRouterHonchoFactory;
    assert_eq!(factory.name(), "apexrouter-honcho");
}
...
fn factory_build_returns_a_real_plugin() {
    ...
    assert_eq!(plugin.manifest().plugin.name, "apexrouter-honcho");
...
async fn initialize_registers_honcho_user_model_spec() {
    ...
    assert_eq!(
        ...
        1,
        "apexrouter-honcho must register exactly one user_model"
    );
