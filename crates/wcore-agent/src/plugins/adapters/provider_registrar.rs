...
//!
//! 1. `apexrouter-ollama`'s `ApexRouterOllama::initialize` registers an
...
//! it's the only crate that links both `apexrouter-ollama` and
//! `wcore-providers` together — `wcore-agent` deliberately doesn't
//! depend on `apexrouter-ollama`. Covered end-to-end by
