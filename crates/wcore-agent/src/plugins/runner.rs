...
/// backend client (e.g. `apexrouter_honcho::HonchoClient`) lands in Task 2.2.
...
pub struct InitializeOutcome {
    ...
    /// Wave BR — count of `BrowserToolSpec`s captured from `apexrouter-browser`
    ...
    /// Wave CU — count of `CuaToolSpec`s captured from `apexrouter-cua`
...
pub struct PluginRunner {
    ...
    /// Wave BR — captures `BrowserToolSpec`s from apexrouter-browser plugins.
    pub browser: HostBrowserRegistrar,
    /// Wave CU — captures `CuaToolSpec`s from apexrouter-cua plugins.
    pub cua: HostCuaRegistrar,
    /// v0.6.4 Task 2.1 — captures `UserModelSpec`s from plugins that
    /// supply a user-model backend (e.g. apexrouter-honcho).
