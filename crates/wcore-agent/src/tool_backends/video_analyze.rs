...
//! `video_analyze` at all. The ApexRouter strategy is a three-step pipeline:
//!
...
//!    path attacker leakage that an open `/tmp/apexrouter-vid-<uuid>-frame-…`
//!    layout enabled).
...
///     `~/.apexrouter/videos/`). The verify-after-canonicalize order is
...
pub fn validate_local_path(raw: &Path) -> Result<PathBuf, String> {
    ...
    if let Some(home) = dirs::home_dir() {
        ...
        let apexrouter = home.join(".apexrouter").join("videos");
        if let Ok(p) = std::fs::canonicalize(&apexrouter) {
            allowed.push(p);
        }
    }
    ...
    if !allowed.iter().any(|p| canonical.starts_with(p)) {
        return Err(format!(
            "video path is outside permitted prefixes (/tmp/, ~/Downloads/, ~/.apexrouter/videos/): {}",
            canonical.display()
        ));
    }
...
///     disk-fill the host.
///
/// The tempfile is created under `~/.apexrouter/videos/` — one of the three
...
async fn download_remote_video(url: &str) -> Result<tempfile::NamedTempFile, VideoAnalysisError> {
    ...
    // Write into `~/.apexrouter/videos/` — one of the three prefixes
    ...
    let videos_dir = dirs::home_dir()
        .map(|h| h.join(".apexrouter").join("videos"))
        .ok_or_else(|| {
...
impl FfmpegFrameVideoBackend {
    ...
    async fn analyze_inner(
        ...
        // fresh, randomized path (no predictable `/tmp/apexrouter-vid-<uuid>-
...
mod tests {
    ...
    fn video_rejects_input_path_outside_permitted_prefixes() {
        ...
        // whitelist (/tmp, ~/Downloads, ~/.apexrouter/videos).
