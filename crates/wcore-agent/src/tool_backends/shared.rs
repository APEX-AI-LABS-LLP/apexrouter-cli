...
mod tests {
    ...
    fn read_env_key_returns_none_for_unset() {
        ...
        let v = std::env::var("APEXROUTER_CLI_TEST_DEFINITELY_UNSET_12345").ok();
        assert!(v.is_none() || v.as_deref() == Some(""));
        assert!(read_env_key("APEXROUTER_CLI_TEST_DEFINITELY_UNSET_12345").is_none());
    }
    ...
    fn read_env_key_returns_none_for_empty() {
        ...
        unsafe { std::env::set_var("APEXROUTER_CLI_TEST_EMPTY_KEY_VAR", "") };
        assert_eq!(read_env_key("APEXROUTER_CLI_TEST_EMPTY_KEY_VAR"), None);
        unsafe { std::env::set_var("APEXROUTER_CLI_TEST_EMPTY_KEY_VAR", "   ") };
        assert_eq!(read_env_key("APEXROUTER_CLI_TEST_EMPTY_KEY_VAR"), None);
        unsafe { std::env::remove_var("APEXROUTER_CLI_TEST_EMPTY_KEY_VAR") };
    }

    #[test]
    fn read_env_key_returns_some_for_set_nonempty() {
        unsafe { std::env::set_var("APEXROUTER_CLI_TEST_NONEMPTY_KEY_VAR", "secret123") };
        assert_eq!(
            read_env_key("APEXROUTER_CLI_TEST_NONEMPTY_KEY_VAR"),
            Some("secret123".to_string())
        );
        unsafe { std::env::remove_var("APEXROUTER_CLI_TEST_NONEMPTY_KEY_VAR") };
    }
