...
//! `$APEXROUTER_CLI_HOME/cron/jobs.json` with atomic tempfile+rename writes),
...
pub struct ApexRouterCronScheduler {
    store: Arc<dyn CronStore>,
}

impl ApexRouterCronScheduler {
...
/// `$APEXROUTER_CLI_HOME` nor `$HOME` resolve (extremely rare; matches the
    ...
    match FileCronStore::from_default_path() {
        Ok(store) => {
            ...
            info!(
                ...
                "cronjob: wiring ApexRouterCronScheduler over default FileCronStore"
            );
            Some(Arc::new(ApexRouterCronScheduler::new(store)))
        }
        Err(e) => {
            warn!(
                target: "wcore_agent::tool_backends::cron",
...
#[async_trait]
impl CronScheduler for ApexRouterCronScheduler {
...
mod tests {
    ...
    async fn cron_persistence_round_trips_through_json() {
        ...
        let sched = ApexRouterCronScheduler::new(store.clone());
        ...
        let fresh_sched = ApexRouterCronScheduler::new(fresh_store);
