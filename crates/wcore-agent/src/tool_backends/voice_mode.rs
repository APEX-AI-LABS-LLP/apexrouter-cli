...
//!   (`$TMPDIR/apexrouter-voice-*`, `~/.apexrouter/voice/`). `..` segments
...
fn permitted_prefixes() -> Vec<PathBuf> {
    ...
    if let Some(home) = dirs::home_dir() {
        v.push(home.join(".apexrouter").join("voice"));
    }
    v
}
...
fn validate_voice_path(path: &Path) -> Result<PathBuf, String> {
    ...
    let canonical_prefixes: Vec<PathBuf> = prefixes
        .into_iter()
        .filter_map(|p| {
            // Best-effort: missing prefixes are silently skipped (~/.apexrouter/voice
    ...
    if !allowed {
        return Err(format!(
            "voice WAV path '{}' is outside permitted prefixes ($TMPDIR/apexrouter-voice-*, ~/.apexrouter/voice/)",
            path.display()
        ));
    }
    Ok(canonical_parent)
...
/// Build a unique WAV path under `$TMPDIR/apexrouter-voice-<nonce>.wav`.
...
fn fresh_wav_path() -> PathBuf {
    let nonce: u64 = std::time::SystemTime::now()
        ...
        .unwrap_or(0);
    std::env::temp_dir().join(format!("apexrouter-voice-{nonce}.wav"))
}
...
impl CpalAudioRecorder {
    ...
    fn spawn_audio_thread(&self) -> Result<std_mpsc::Sender<AudioControl>, String> {
        ...
        let handle = thread::Builder::new()
            .name("apexrouter-voice-audio".to_string())
            .spawn(move || {
...
mod tests {
    ...
    fn wav_path_under_permitted_prefix_only() {
        // /tmp/apexrouter-voice-*.wav under $TMPDIR is OK.
        let ok = std::env::temp_dir().join("apexrouter-voice-test-b10.wav");
        let _ = std::fs::remove_file(&ok);
        assert!(validate_voice_path(&ok).is_ok());

        // /etc is outside permitted prefixes — rejected.
        let bad = PathBuf::from("/etc/apexrouter-voice-evil.wav");
    ...
    async fn voice_mode_loop_uses_existing_transcription_backend() {
        ...
        let path = PathBuf::from("/tmp/apexrouter_voice_b10_test_capture.wav");
        let out =
    ...
    async fn tts_reply_uses_b2_backend() {
        ...
        let p = PathBuf::from("/tmp/apexrouter_voice_test_b10_tts.wav");
    ...
    async fn cpal_player_returns_false_on_missing_file() {
        ...
        let bogus = PathBuf::from("/nonexistent/apexrouter-voice-doesnt-exist.wav");
        assert!(
