//! v0.9.0 Wave-1 B7 — in-process backend for `apexrouter_status` +
...
//!
//! `apexrouter_telemetry_query` lives in the default `tools.allow_list`
...
use wcore_tools::apexrouter_introspection::{
    IntrospectionOutcome, StatusQuery, TelemetryQuery as ToolTelemetryQuery,
    ApexRouterIntrospectionBackend,
};
...
/// Typed shape for `apexrouter_telemetry_query.query`. JSON shape is
...
#[async_trait]
impl ApexRouterIntrospectionBackend for LocalIntrospectionBackend {
...
/// Returns an `Arc<dyn ApexRouterIntrospectionBackend>` ready to hand to
/// `ApexRouterStatusTool::new` / `ApexRouterTelemetryQueryTool::new`.
pub fn build_introspection_backend(
    state: Arc<dyn SessionStateReader>,
) -> Arc<dyn ApexRouterIntrospectionBackend> {
