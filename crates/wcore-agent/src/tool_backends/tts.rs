...
fn permitted_prefixes() -> Vec<PathBuf> {
    ...
    if let Some(home) = dirs::home_dir() {
        v.push(home.join(".apexrouter").join("tts"));
...
fn validate_output_path(path: &Path) -> Result<PathBuf, TtsError> {
    ...
    if !allowed {
        return Err(TtsError::Configuration(format!(
            "output_path '{}' is outside permitted prefixes (temp dir, ~/.apexrouter/tts/, $HOME)",
            path.display()
        )));
    }
    Ok(canonical_parent)
...
impl OpenAiTtsBackend {
    ...
    pub(crate) fn with_endpoint(api_key: String, endpoint: String) -> Self {
        let model =
            std::env::var("APEXROUTER_CLI_OPENAI_TTS_MODEL").unwrap_or_else(|_| "tts-1".to_string());
        Self {
...
impl ElevenLabsTtsBackend {
    ...
    pub(crate) fn with_endpoint(api_key: String, endpoint_base: String) -> Self {
        let model_id = std::env::var("APEXROUTER_CLI_ELEVENLABS_MODEL")
...
mod tests {
    ...
    fn make_request(output_path: PathBuf) -> TtsRequest {
        TtsRequest {
            text: "hello from apexrouter".to_string(),
            provider: TtsProvider::OpenAi,
            voice: None,
            model: None,
    ...
    async fn tts_rejects_output_path_outside_permitted_prefix() {
        // /etc is not under temp dir, $HOME, or ~/.apexrouter/tts/.
        // Most CIs don't have /etc/apexrouter-tts-test writable but that's
