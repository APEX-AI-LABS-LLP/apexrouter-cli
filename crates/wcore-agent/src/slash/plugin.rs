...
///   standalone `apexrouter-cli plugin {install,remove}` CLI because
...
impl PluginHandler {
            ...
            Self::Stub => Ok(SlashOutcome::Handled {
                output: Some(
                    "/plugin list: full plugin inventory needs the runtime \
                     PluginRegistry handle; use `apexrouter-cli plugin list` \
    ...
    fn install(&self, name: &str) -> Result<SlashOutcome, SlashError> {
        match self {
            Self::Stub => Ok(SlashOutcome::Handled {
                output: Some(format!(
                    "use `apexrouter-cli plugin install {name}` from the CLI in v0.7.0"
                )),
            }),
            Self::Runtime { .. } => Ok(SlashOutcome::Handled {
                output: Some(format!(
                    "/plugin install {name}: install records live on disk and require \
                     ...
                     Run `apexrouter-cli plugin install {name}` from another \
    ...
    fn remove(&self, name: &str) -> Result<SlashOutcome, SlashError> {
        match self {
            Self::Stub => Ok(SlashOutcome::Handled {
                output: Some(format!(
                    "use `apexrouter-cli plugin remove {name}` from the CLI in v0.7.0"
                )),
            }),
            Self::Runtime { .. } => Ok(SlashOutcome::Handled {
                output: Some(format!(
                    "/plugin remove {name}: install records live on disk; \
                     run `apexrouter-cli plugin remove {name}` from another \
