...
pub struct HookOutcome {
    ...
    /// caused `[plugin-hook:apexrouter-ijfw:...] post_tool_use fired for ...`
...
mod c1_dispatch_proof {
    ...
    async fn contribution_is_an_untrusted_user_block() {
        let mut engine = engine_with_session_hook("apexrouter-ijfw");
        ...
        assert!(
            text.contains("source=\"apexrouter-ijfw:ijfw_memory_prelude\""),
    ...
    async fn no_dispatcher_preserves_log_only_behavior() {
        let engine = engine_with_session_hook("apexrouter-ijfw");
        let outcome = engine.run_session_start().await;
    ...
    async fn pre_prompt_contribution_is_an_untrusted_user_block() {
        ...
        assert!(
            text.contains("source=\"apexrouter-ijfw:ijfw_memory_recall\""),
            "missing provenance: {text}"
        );
        assert!(
    ...
    async fn turn_start_contribution_is_an_untrusted_user_block() {
        let mut engine = engine_with_hook("apexrouter-ijfw", HookPhase::TurnStart, "ijfw_turn_start");
        engine.set_dispatcher(Arc::new(StubDispatcher {
        ...
        assert!(
            text.contains("source=\"apexrouter-ijfw:ijfw_turn_start\""),
            "missing provenance: {text}"
        );
        assert!(
    ...
    async fn turn_end_contribution_is_an_untrusted_user_block() {
        let mut engine = engine_with_hook("apexrouter-ijfw", HookPhase::TurnEnd, "ijfw_turn_end");
        engine.set_dispatcher(Arc::new(StubDispatcher {
        ...
        assert!(
            text.contains("source=\"apexrouter-ijfw:ijfw_turn_end\""),
            "missing provenance: {text}"
        );
        assert!(
    ...
    async fn post_tool_use_contribution_is_an_untrusted_user_block() {
        let mut engine =
            engine_with_hook("apexrouter-ijfw", HookPhase::PostToolUse, "ijfw_observation");
        engine.set_dispatcher(Arc::new(StubDispatcher {
        ...
        assert!(
            text.contains("source=\"apexrouter-ijfw:ijfw_observation\""),
            "missing provenance: {text}"
        );
        assert!(
