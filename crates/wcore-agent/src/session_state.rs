//! v0.9.0 Wave-1 B7 — live engine state accessor for `apexrouter_status` +
...
/// A single error captured by the engine for `apexrouter_status` recent-error
...
pub trait SessionStateReader: Send + Sync + 'static {
    ...
    /// When the current session started (used by `apexrouter_status` to
...
/// `apexrouter_telemetry_query` can ask for larger windows without
