...
//!   `<config_dir>/apexrouter-cli/skills/auto-<sig>/SKILL.md`  — loader-visible
//!   `<config_dir>/apexrouter-cli/skills/auto-<sig>/manifest.json` — metadata
//!
//! A secondary write to `self.skill_dir` (APEXROUTER_CLI_HOME-based) is attempted as
...
impl SkillDrafter {
    ...
    ///
    /// 1. **Loader-visible path** — `<config_dir>/apexrouter-cli/skills/auto-<sig>/SKILL.md`
    ///    (matches `user_skills_dir()` in `wcore-skills::paths`).
    /// 2. **Legacy path** — `<self.skill_dir>/auto-<sig>/SKILL.md` (the path
    ///    bootstrap wires from `$APEXROUTER_CLI_HOME/skills/auto/`). Written as a
    ...
    pub fn draft(&self, trigger: &DraftTrigger) -> Result<DraftResult, DraftError> {
        ...
        // This matches `user_skills_dir()` = `<config_dir>/apexrouter-cli/skills/`.
        ...
        // Secondary write: legacy path under self.skill_dir (APEXROUTER_CLI_HOME/skills/auto/).
