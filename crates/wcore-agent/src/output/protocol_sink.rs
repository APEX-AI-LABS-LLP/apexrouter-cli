...
/// set, so a malicious crate with `name = "apexrouter-browser"` in its
...
pub struct PluginCapabilitySet {
    /// True when `apexrouter-browser` is among the loaded plugins AND
    /// the manifest's identity was verified.
    pub browser_suite: bool,
    /// True when `apexrouter-cua` is among the loaded plugins AND
    /// the manifest's identity was verified.
...
impl PluginCapabilitySet {
    ...
    /// `name = "apexrouter-browser"` would impersonate the real
    ...
    pub fn from_loaded(names: &[String]) -> Self {
        ...
        Self {
            browser_suite: names.iter().any(|n| n == "apexrouter-browser"),
            computer_use: names.iter().any(|n| n == "apexrouter-cua"),
        }
    }
    ...
    /// `name = "apexrouter-browser"` shipping outside the static inventory
    ...
    pub fn from_verified(loaded: &[(String, wcore_plugin_api::PluginIdentity)]) -> Self {
        ...
        Self {
            browser_suite: verified("apexrouter-browser"),
            computer_use: verified("apexrouter-cua"),
        }
    }
}
