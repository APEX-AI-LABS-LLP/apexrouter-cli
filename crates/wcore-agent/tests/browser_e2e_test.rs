//! Wave BR E2E — boot the plugin loader, discover apexrouter-browser, run
...
//!
//! NOTE: wcore-agent doesn't link apexrouter-browser directly (that would
...
//! the SAME `register_browser_tool` API path that apexrouter-browser uses.
//! The full discovery + linking-via-inventory smoke test is the existing
//! `crates/apexrouter-browser/tests/plugin_load_test.rs` + the wcore-cli
...
fn fixture_manifest(allowed: &[&str]) -> PluginManifest {
    PluginManifest {
        plugin: PluginInfo {
            name: "apexrouter-browser".into(),
            version: "0.1.0".into(),
            description: "test mirror".into(),
            entry: Some("builtin:apexrouter_browser".into()),
            authors: vec![],
            license: "MIT".into(),
            deferred: false,
        },
...
#[tokio::test]
async fn host_registrar_captures_apexrouter_browser_spec() {
    // Mirrors the apexrouter-browser plugin's `initialize()` body: a plugin
