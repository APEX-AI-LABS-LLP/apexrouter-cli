...
async fn registered_plugin_tool_is_invocable_through_the_full_pipeline() {
    ...
    assert!(
        discovered.iter().any(|p| p.name() == "apexrouter-e2e-fixture"),
    ...
    assert!(
        outcome
            .tools
            .iter()
            .any(|t| t.tool.name == "echo_fixture" && t.plugin == "apexrouter-e2e-fixture"),
    ...
    let result = tool
        .execute(serde_json::json!({ "text": "apexrouter-e2e" }))
        .await;
    assert!(!result.is_error, "tool execution must succeed: {result:?}");
    assert_eq!(
        result.content, "apexrouter-e2e",
