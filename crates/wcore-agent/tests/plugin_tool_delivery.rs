...
async fn plugin_tool_is_captured_then_reified_into_a_working_tool() {
    ...
    {
        let mut capture = host.capture_for_plugin("apexrouter-toolful");
        let mut scoped =
    ...
    assert_eq!(plugin, "apexrouter-toolful", "provenance plugin name stamped");
    assert_eq!(
        fq_name, "ijfw::shout",
    ...
    assert_eq!(captured.plugin, "apexrouter-toolful");
    ...
    assert_eq!(reified.plugin, "apexrouter-toolful");
    assert_eq!(reified.fq_name, "ijfw::shout");
    let boxed: Box<dyn Tool> = reified.tool;
    assert_eq!(boxed.name(), "shout");
    let result = boxed
        .execute(serde_json::json!({ "text": "apexrouter" }))
        .await;
    assert_eq!(result.content, "APEXROUTER");
}
