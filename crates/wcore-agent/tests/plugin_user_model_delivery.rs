...
//! (`apexrouter-honcho` reifies it into a live `UserModel` client during
//! bootstrap).
//!
...
fn captured_user_model_survives_apply() {
    ...
    outcome
        .user_models
        .push(captured("apexrouter-honcho", "honcho"));
    ...
    assert_eq!(got.plugin, "apexrouter-honcho");
...
fn honcho_user_model_reifies() {
    let key_var = "APEXROUTER_CLI_TASK_1_5_HONCHO_KEY";
    ...
    outcome.user_models.push(CapturedUserModel {
        plugin: "apexrouter-honcho".to_string(),
    ...
    assert_eq!(reified.plugin, "apexrouter-honcho");
...
fn honcho_missing_api_key_degrades_silently() {
    let key_var = "APEXROUTER_CLI_TASK_1_5_DEFINITELY_UNSET_KEY";
