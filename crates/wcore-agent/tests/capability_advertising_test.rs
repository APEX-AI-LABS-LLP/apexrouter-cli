...
#[test]
fn browser_suite_advertised_when_apexrouter_browser_loaded() {
    let names = vec!["apexrouter-browser".to_string()];
...
#[test]
fn computer_use_advertised_when_apexrouter_cua_loaded() {
    let names = vec!["apexrouter-cua".to_string()];
    let caps = PluginCapabilitySet::from_loaded(&names);
    assert!(caps.computer_use);
    assert!(!caps.browser_suite);
...
fn unknown_plugin_names_do_not_flip_known_flags() {
    let names = vec!["apexrouter-totally-unknown".to_string()];
    let caps = PluginCapabilitySet::from_loaded(&names);
    assert!(!caps.browser_suite);
    assert!(!caps.computer_use);
}
