...
/// `APEXROUTER_CLI_PLUGINS_DIR` / `APEXROUTER_CLI_PLUGIN_TRUST_UNSIGNED` are global
...
/// Pin `$APEXROUTER_CLI_PLUGINS_DIR` for the duration of a test. Helper
...
impl Drop for EnvGuard {
    fn drop(&mut self) {
        unsafe {
            std::env::remove_var("APEXROUTER_CLI_PLUGINS_DIR");
            std::env::remove_var("APEXROUTER_CLI_PLUGIN_TRUST_UNSIGNED");
        }
    }
}
...
//   2. A symlink in `~/.apexrouter/plugins/<name>` pointing at an
...
// is scanned in ADDITION to `<data_dir>/apexrouter/plugins` when the
// `$APEXROUTER_CLI_PLUGINS_DIR` override is NOT set. This exercises the multi-root
// branch of `resolved_plugins_roots()` — a declarative plugin installed by
// IJFW's installer under `~/.apexrouter/plugins` must be discovered.
// ---------------------------------------------------------------------------

/// Pin `$APEXROUTER_CLI_HOME` (so `profile_home()` resolves to a tempdir) while
/// ensuring `$APEXROUTER_CLI_PLUGINS_DIR` is UNSET, so discovery takes the multi-root
...
impl ProfileHomeEnvGuard {
    fn set(home: &Path) -> Self {
        let lock = env_mutex().lock().unwrap_or_else(|p| p.into_inner());
        let saved_plugins_dir = std::env::var_os("APEXROUTER_CLI_PLUGINS_DIR");
        unsafe {
            // Force the multi-root branch: no single-dir override.
            std::env::remove_var("APEXROUTER_CLI_PLUGINS_DIR");
            std::env::set_var("APEXROUTER_CLI_HOME", home);
...
impl Drop for ProfileHomeEnvGuard {
    fn drop(&mut self) {
        unsafe {
            std::env::remove_var("APEXROUTER_CLI_HOME");
            std::env::remove_var("APEXROUTER_CLI_PLUGIN_TRUST_UNSIGNED");
            match self.saved_plugins_dir.take() {
                Some(v) => std::env::set_var("APEXROUTER_CLI_PLUGINS_DIR", v),
                None => std::env::remove_var("APEXROUTER_CLI_PLUGINS_DIR"),
            }
        }
    }
}
...
/// path, WITHOUT setting `$APEXROUTER_CLI_PLUGINS_DIR`.
#[tokio::test]
async fn on_disk_declarative_plugin_under_profile_home_discovered() {
    use wcore_agent::plugins::LoadedRuntimeHandle;

    // profile_home() == $APEXROUTER_CLI_HOME (directly), so the plugins root is
    // `<home>/plugins`.
