...
mod tests {
    ...
    fn project_root_returns_host_advertised_path() {
        let host = FakeHost {
            project_root: Some(PathBuf::from("/var/projects/apexrouter")),
            ..FakeHost::default()
        };
        let reader = ScopedConfigReader::new(&host);
        ...
        assert_eq!(
            reader.project_root(),
            Some(PathBuf::from("/var/projects/apexrouter"))
        );
    }
    ...
    fn plugin_config_returns_namespaced_bucket_when_plugin_name_set() {
        ...
        let reader = ScopedConfigReader::with_plugin_name(&host, "apexrouter-ijfw");
