...
fn dummy_plugin_discoverable_via_inventory() {
    ...
    assert!(
        names.contains(&"apexrouter-test-dummy"),
...
fn discovered_factory_builds_a_working_plugin() {
    let factory = plugin_inventory::iter()
        .find(|f| f.name() == "apexrouter-test-dummy")
        .expect("dummy plugin not found");
    let plugin = factory.build();
    assert_eq!(plugin.manifest().plugin.name, "apexrouter-test-dummy");
...
[plugin]
name = "apexrouter-test-bad-version"
version = "0.0.1"
...
"#,
    ...
    match err {
        PluginError::VersionMismatch {
            ...
            assert_eq!(plugin, "apexrouter-test-bad-version");
...
[plugin]
name = "apexrouter-test-good-version"
version = "0.0.1"
...
fn missing_version_is_compatible() {
    ...
    let manifest = PluginManifest::from_toml_str(
        r#"
[plugin]
name = "apexrouter-test-no-version"
version = "0.0.1"
...
fn unknown_kind_rejects() {
    let err = PluginManifest::from_toml_str(
        r#"
[plugin]
name = "apexrouter-test-bad-runtime"
version = "0.0.1"
...
"#,
    ...
    match err {
        PluginError::UnknownRuntimeKind { plugin, kind } => {
            assert_eq!(plugin, "apexrouter-test-bad-runtime");
            assert_eq!(kind, "wat");
        }
...
fn default_runtime_kind_is_static_implicit() {
    ...
    let m = PluginManifest::from_toml_str(
        r#"
[plugin]
name = "apexrouter-test-default-runtime"
version = "0.0.1"
...
fn explicit_static_runtime_accepted() {
    let m = PluginManifest::from_toml_str(
        r#"
[plugin]
name = "apexrouter-test-static-runtime"
version = "0.0.1"
