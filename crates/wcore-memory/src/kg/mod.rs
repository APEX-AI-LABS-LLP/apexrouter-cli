...
//! nodes/edges at session/turn end. The `APEXROUTER_CLI_KG=off` rollback flag
//! disables KG init.
//!
...
//!
//! Rollback: set `APEXROUTER_CLI_KG=off` (callers gate `kg::init` on this env var)
...
pub const ENV_KG: &str = "APEXROUTER_CLI_KG";

/// Returns `true` unless `APEXROUTER_CLI_KG` is set to (case-insensitive) `"off"`.
