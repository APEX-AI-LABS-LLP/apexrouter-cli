...
//! `APEXROUTER_CLI_STALENESS=off` rollback flag disables the cascade.
//!
...
//!   )
//!
//! Rollback: set APEXROUTER_CLI_STALENESS=off to skip propagation; mark_stale
...
pub const ENV_STALENESS: &str = "APEXROUTER_CLI_STALENESS";

/// Returns `true` unless `APEXROUTER_CLI_STALENESS` is set to (case-insensitive) `"off"`.
...
mod tests {
    ...
    //
    // `APEXROUTER_CLI_STALENESS` is process-global; tests serialize via the
