...
//! `apexrouter_config_dir()/auto-memorize.consent`, or with the
//! `APEXROUTER_CLI_AUTO_MEMORIZE=off` env kill switch. Resolved via
//! `wcore_config::config::apexrouter_config_dir()` so `APEXROUTER_CLI_HOME`
...
//! # Rollback
//!
//! Set `APEXROUTER_CLI_AUTO_MEMORIZE=off` in the environment to make
...
pub const ENV_AUTO_MEMORIZE: &str = "APEXROUTER_CLI_AUTO_MEMORIZE";
...
/// `APEXROUTER_CLI_AUTO_MEMORIZE` env var is not set to `"off"`).
///
/// Resolution: `wcore_config::config::apexrouter_config_dir()` so
/// `APEXROUTER_CLI_HOME` hermetically sandboxes the consent flag alongside the
/// rest of the engine's on-disk state (F-010, #270). The helper has its
/// own `PathBuf::from("apexrouter-cli")` fallback when the platform exposes
/// no config dir, so this function never panics.
pub fn consent_file_path() -> PathBuf {
    wcore_config::config::apexrouter_config_dir().join(CONSENT_FILE_NAME)
}
...
///
/// Decision order:
///   1. If `APEXROUTER_CLI_AUTO_MEMORIZE=off` (case-insensitive), return `false`
...
///
/// Honors the `APEXROUTER_CLI_AUTO_MEMORIZE=off` env override the same way as
...
/// Returns `true` when `APEXROUTER_CLI_AUTO_MEMORIZE` is set to `"off"`
...
pub enum SkipReason {
    /// Consent file missing (or `APEXROUTER_CLI_AUTO_MEMORIZE=off`).
    ConsentNotGranted,
...
mod tests {
    ...
    /// (`APEXROUTER_CLI_AUTO_MEMORIZE`) is process-global.
    ...
    fn consent_file_path_ends_in_expected_components() {
        ...
        // The path flows through `apexrouter_config_dir()`, which always
        // ends in the `apexrouter-cli` segment (or its `APEXROUTER_CLI_HOME`
        // override). Asserting on `apexrouter-cli` locks the helper as the
        // canonical resolver (F-010, #270).
        assert!(
            s.contains("apexrouter-cli") || std::env::var_os("APEXROUTER_CLI_HOME").is_some(),
            "consent path {s:?} should include 'apexrouter-cli' subdir when APEXROUTER_CLI_HOME is unset"
        );
        assert!(
    ...
    #[test]
    #[serial(env)]
    fn consent_file_path_honors_apexrouter_home() {
        // Hermeticity: when APEXROUTER_CLI_HOME is set the consent file MUST
        // resolve inside that sandbox, not the user's real config dir
        // (F-010, #270). Same regression class as F-019.
        let _g = env_lock().lock().unwrap();
        let prior = std::env::var("APEXROUTER_CLI_HOME").ok();
        let tmp = tempfile::tempdir().unwrap();
        // SAFETY: env_lock + #[serial(env)] serialize all env writes.
        unsafe {
            std::env::set_var("APEXROUTER_CLI_HOME", tmp.path());
        }

        let p = consent_file_path();
        assert!(
            p.starts_with(tmp.path()),
            "consent path {:?} should be rooted at APEXROUTER_CLI_HOME ({:?})",
            p,
            tmp.path()
        );
    ...
    fn run_session_end_persists_when_consent_granted_and_facts_above_threshold() {
        ...
        let digest = digest_with(vec![
            ...
            fact("u", "works-on", "apexrouter", 0.8),
        ]);
    ...
    fn run_session_end_below_confidence_threshold() {
        ...
        let digest = digest_with(vec![
            ...
            fact("u", "works-on", "apexrouter", 0.20),
        ]);
    ...
    fn run_session_end_below_fact_threshold() {
        ...
        let digest = digest_with(vec![
            ...
            fact("u", "works-on", "apexrouter", 0.95), // survives
