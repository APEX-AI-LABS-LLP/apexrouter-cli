...
Scenario-level eval harness for `apexrouter-cli`. Drives the real shipped binary against a real LLM API through a real tool chain and asserts the OUTCOME — not just that the tools ran.
...
- **Crate scaffold** — public API types (`Scenario`, `Turn`, `Assertion`, `TraceAssertion`, `ScenarioResult`, `Failure`, `ProviderChoice`, `Category`), workspace wiring, `[profile.eval]` nextest profile, `apexrouter-eval` bin stub.
- **Runner core** — spawn `apexrouter-cli --json-stream`, drive per-turn via `message` / `stream_end` events, capture stderr to a 50-line ring buffer, parse `session_cost` for USD totals, enforce wall-time with `kill_on_drop(true)` + explicit `start_kill()` on `Elapsed` (per cross-audit M-1).
- **`tempenv`** — hermetic per-scenario `TempDir` + seeded `<tempdir>/.apexrouter-cli/config.toml` with an **absolute** `[session].directory` (per C-3 — relative defaults leak into cwd) and the per-provider API key.
...
- **T5** — `report::render_console / render_markdown / render_json` + the real `apexrouter-eval` CLI.
