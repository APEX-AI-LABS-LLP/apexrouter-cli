//! D8 — live PTY smoke test: boot the real `apexrouter-cli` TUI under a
...
//!
//! ```text
//! WCORE_EVAL_BIN=$PWD/target/release/apexrouter-cli \
...
//! never makes an LLM call. `PtyCapture` points `APEXROUTER_CLI_HOME`/`HOME` at a
...
#[test]
#[ignore = "live: boots the real apexrouter-cli TUI under a PTY (needs a pre-built binary)"]
fn tui_boots_and_renders_workspace() {
    ...
    if discover_binary().is_err() {
        eprintln!(
            "SKIP tui_boots_and_renders_workspace: no apexrouter-cli binary. \
    ...
    let mut cap = PtyCapture::spawn(&provider).expect("spawn apexrouter-cli TUI under a PTY");

    // The core D8 assertion: the workspace chrome (the APEXROUTER wordmark AND the
    ...
    cap.wait_for_workspace()
        .expect("TUI should render the workspace chrome (APEXROUTER wordmark + Workspace tab)");
