...
//!    spawn helpers can launch `apexrouter-cli --help`, see exit 0, and
...
/// at `target/{debug,release}/apexrouter-cli` — neither of which is
...
fn maybe_binary() -> Option<std::path::PathBuf> {
    match discover_binary() {
        ...
        Err(e) => {
            eprintln!(
                "[smoke] skipping: could not locate apexrouter-cli binary: {e}. \
...
async fn spawns_and_captures_help() {
    ...
    assert!(
        status.success(),
        "apexrouter-cli --help should exit 0, got {status}"
    );
    let s = String::from_utf8_lossy(&buf);
    assert!(!s.trim().is_empty(), "apexrouter-cli --help stdout was empty");
    ...
    assert!(
        s.contains("apexrouter-cli") || s.to_lowercase().contains("usage"),
        "help stdout missing both 'apexrouter-cli' and 'usage' anchors:\n{s}"
    );
}
...
fn binary_discovery_rejects_missing_override() {
    let guard = EnvGuard::set("WCORE_EVAL_BIN", "/nonexistent/path/to/apexrouter-cli");
