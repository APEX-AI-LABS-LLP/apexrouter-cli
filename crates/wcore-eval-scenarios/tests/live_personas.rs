...
//!
//! This drives the REAL `apexrouter-cli` binary against the real DeepSeek
...
//!
//! ```text
//! APEXROUTER_CLI_ALLOW_NO_SANDBOX=1 DEEPSEEK_API_KEY=... \
...
#[ignore = "live: drives the real apexrouter-cli binary against the real DeepSeek API (costs money, needs DEEPSEEK_API_KEY + a pre-built binary)"]
async fn overnight_personas() {
    ...
    //    target/{release,debug}/apexrouter-cli. Require it — a live run with no
    //    binary is operator error, so panic with an actionable message.
    let bin = match discover_binary() {
        Ok(p) => p,
        Err(e) => panic!(
            "apexrouter-cli binary not found ({e}). \
