...
/// Write a POSIX shell script to `path` that mimics a `apexrouter-cli
...
fn write_fake_binary(path: &std::path::Path) {
    ...
    let script = r#"#!/bin/sh
# Mock apexrouter-cli --json-stream for runner cost-capture regression (#278).
...
async fn captures_session_cost_emitted_before_stream_end() {
    ...
    let fake = tmp.path().join("apexrouter-cli");
