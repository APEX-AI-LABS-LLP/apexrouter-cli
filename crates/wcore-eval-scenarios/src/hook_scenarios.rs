...
//!
//! These exercise apexrouter-cli's **lifecycle hooks** (the `[[hooks.*]]`
...
//!
//! `tempenv::build` writes `<cwd>/.apexrouter-cli/config.toml` (DIR form) with
//! the absolute `[session].directory` + provider key. `config.rs`
//! `project_config_path()` PREFERS `.apexrouter-cli.toml` (FILE form) when BOTH
...
//! exactly that (open-append `<cwd>/.apexrouter-cli/config.toml`).
//!
...
const CONFIG_REL: &str = ".apexrouter-cli/config.toml";
