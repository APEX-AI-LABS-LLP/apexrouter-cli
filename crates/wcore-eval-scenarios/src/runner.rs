...
//!
//! Spawn `apexrouter-cli --json-stream`, drive per-turn via
...
#[derive(Debug, Error)]
pub enum SpawnError {
    #[error("could not locate apexrouter-cli binary: {0}")]
    BinaryMissing(String),
    #[error(transparent)]
    Io(#[from] std::io::Error),
}

/// Locate the `apexrouter-cli` binary the runner should spawn.
///
/// Resolution order (first hit wins):
/// 1. `WCORE_EVAL_BIN` env var — explicit override; tests can pin a
///    specific build.
/// 2. `target/release/apexrouter-cli` then `target/debug/apexrouter-cli`
///    by walking up from `CARGO_MANIFEST_DIR` two levels (mirrors the
///    pattern in `crates/wcore-cli/tests/release_binary_smoke.rs`).
///
/// `CARGO_BIN_EXE_apexrouter-cli` is NOT available here — Cargo only
...
pub fn discover_binary() -> Result<PathBuf, SpawnError> {
    ...
    Err(SpawnError::BinaryMissing(format!(
        "no apexrouter-cli binary at {}/target/{{release,debug}}/{bin_name}; \
         pre-build it (`cargo build -p wcore-cli`) or set WCORE_EVAL_BIN",
        workspace_root.display()
    )))
}

/// Spawn `apexrouter-cli` configured for a scenario run — `--yolo`
...
pub fn spawn_for_run(
    ...
    yolo: bool,
    apexrouter_home: Option<&std::path::Path>,
    ...
    cmd.arg("--json-stream")
        ...
        .stderr(Stdio::piped())
        .kill_on_drop(true);
    // `apexrouter_config_dir()` = `$APEXROUTER_CLI_HOME` resolves the global config layer:
    ...
    match apexrouter_home {
        Some(home) => {
            cmd.env("APEXROUTER_CLI_HOME", home);
        }
        None => {
            cmd.env_remove("APEXROUTER_CLI_HOME");
        }
    }
...
/// the `apexrouter-cli --help` smoke test that does NOT touch the engine.
...
pub async fn run(
    ...
    // Persona path: a fresh hermetic throwaway env per run. APEXROUTER_CLI_HOME points
    // at the tempdir (NOT stripped): `apexrouter_config_dir()` then resolves to the
    ...
    // apexrouter-cli` — is NOT loaded. (Stripping the var, the prior behaviour,
    ...
    // live in the cwd-walk PROJECT layer (`<tempdir>/.apexrouter-cli/config.toml`),
    // which loads regardless of APEXROUTER_CLI_HOME, so this only empties the global
...
/// sessions against ONE shared persistent home (`apexrouter_home = Some(home)`),
/// while the persona path keeps its hermetic throwaway env
/// (`apexrouter_home = None`, which strips `APEXROUTER_CLI_HOME`). The caller owns the
...
pub(crate) async fn run_session_in(
    ...
    // `[[hooks.*]]`) onto the tempenv-seeded `.apexrouter-cli/config.toml`. This
