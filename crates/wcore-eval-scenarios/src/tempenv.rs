...
//!
//! Cross-audit C-3 caught the trap: `APEXROUTER_CLI_HOME` does NOT reroute
//! the session directory. `config.session.directory` defaults to the
//! relative string `".apexrouter-cli/sessions"` (`wcore-config/src/config.rs:482-484`).
//! Setting env vars without also `cd`-ing or pointing the config at an
//! absolute path leaks session files into the worktree.
//!
//! [`build`] therefore:
//! 1. Mints a `TempDir`.
//! 2. Writes `<tempdir>/.apexrouter-cli/config.toml` with an **absolute**
...
mod tests {
    ...
    fn seeded_config_has_absolute_session_dir() {
        ...
        let cfg = fs::read_to_string(env.path().join(".apexrouter-cli/config.toml"))
