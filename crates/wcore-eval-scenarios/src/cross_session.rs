...
//! `APEXROUTER_CLI_HOME` *stripped*, so nothing survives between runs — correct for
...
//! a home that persists across two separate `apexrouter-cli` processes.
//!
//! ## The substrate
//! `APEXROUTER_CLI_HOME` → `wcore_config::config::apexrouter_config_dir()` drives BOTH:
//! - the memory DBs (`wcore-memory` resolves its base dir from
//!   `app_config_dir()` = `$APEXROUTER_CLI_HOME`; the **global** tier — `memory.db` —
//!   carries across sessions), and
//! - the skills dir (`$APEXROUTER_CLI_HOME/skills{,/auto}` — where the auto-drafter
...
//! - **`home/`** — `APEXROUTER_CLI_HOME`. Memory + skills live here, deliberately
...
//!   Holds the seeded `.apexrouter-cli/config.toml`.
//!
...
impl CrossSessionEnv {
    /// Build the env: create `home/` + `home/sessions/` + `project/.apexrouter-cli/`
    /// and seed `project/.apexrouter-cli/config.toml`.
    ///
    ...
    pub fn build(provider: &ProviderConfig) -> anyhow::Result<Self> {
        ...
        let cfg_dir = project.join(".apexrouter-cli");
    ...
    /// `APEXROUTER_CLI_HOME` for both sessions — the shared memory + skills root.
...
/// Drive a sequence of scenarios as SEPARATE `apexrouter-cli` processes that all
...
/// same `APEXROUTER_CLI_HOME` + `cwd`, so memory DBs and the skills dir carry over.
...
mod tests {
    ...
    fn seeded_config_enables_memory_with_no_dream_throttle() {
        ...
        let cfg = fs::read_to_string(env.project().join(".apexrouter-cli/config.toml"))
    ...
    fn session_dir_lives_under_home_not_project() {
        // The session store must sit under APEXROUTER_CLI_HOME (shared, and OUTSIDE
        ...
        #[cfg(unix)]
        {
            let cfg = fs::read_to_string(env.project().join(".apexrouter-cli/config.toml"))                .expect("seeded config exists");
    ...
    fn home_and_project_are_distinct() {
        ...
        assert!(env.project().join(".apexrouter-cli").is_dir());
    }
}
