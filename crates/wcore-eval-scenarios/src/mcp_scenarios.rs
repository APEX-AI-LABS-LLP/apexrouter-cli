...
//!
//! Exercises a full MCP stdio round-trip through the REAL `apexrouter-cli`
...
//!
//! apexrouter-cli reads MCP servers ONLY from `[mcp.servers.*]` in
//! `config.toml` (`wcore-config/src/config.rs` `McpConfig` /
//! `McpServerConfig`; there is NO project-level `.mcp.json` reader — grep
//! confirmed). `tempenv::build` seeds `<cwd>/.apexrouter-cli/config.toml`
...
const SENTINEL: &str = "APEXROUTER-MCP-RTRIP-7F3A";
...
///    that `tempenv::build` already wrote at `.apexrouter-cli/config.toml`.
fn setup_mock_mcp(cwd: &std::path::Path) -> anyhow::Result<()> {
    ...
    let config_path = cwd.join(".apexrouter-cli").join("config.toml");
