...
//!
//! The json-stream runner ([`crate::runner`]) drives `apexrouter-cli` in its
...
//! ## Why a PTY
//!
//! `apexrouter-cli` only launches its full-screen TUI when
...
//! tempdir holds a seeded `.apexrouter-cli/config.toml` (absolute session dir per
//! cross-audit C-3, plus the provider id/model/key), the binary is spawned with
//! `cwd = env.path()`, and `APEXROUTER_CLI_HOME` is pointed at the tempdir so
//! `wcore_config::apexrouter_config_dir()` resolves the seeded config on every
...
/// A live `apexrouter-cli` TUI process attached to a pseudo-terminal.
///
...
impl PtyCapture {
    /// Spawn `apexrouter-cli` in interactive TUI mode under a fresh PTY of the
    ...
    /// `target/{release,debug}/apexrouter-cli` walk-up.
    ...
    pub fn spawn_with(
        ...
        let bin = crate::runner::discover_binary()
            .map_err(|e| anyhow!("locate apexrouter-cli binary: {e}"))?;
        ...
        // cwd-walk lands inside the sandbox; APEXROUTER_CLI_HOME = tempdir so
        // `apexrouter_config_dir()` resolves the seeded config on every platform;
        ...
        cmd.cwd(env.path());
        cmd.env("APEXROUTER_CLI_HOME", env.path());
        ...
        let child = pty
            .slave
            .spawn_command(cmd)
            .context("spawn apexrouter-cli under PTY")?;
    ...
    /// `APEXROUTER` wordmark and the `Workspace` tab), the canonical "the UI is up
