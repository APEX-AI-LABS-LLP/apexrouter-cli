...
/// don't pick up the host's `.apexrouter-cli.toml`.
...
fn default_bedrock_when_no_model_uses_alias() {
    ...
    // `apexrouter_config_dir()/config.toml` (macOS: ~/Library/Application
    // Support/apexrouter-cli/config.toml).  On a developer host that has
    // `model = "<something>"` in that file the default-model path is
    // never exercised and the test fails spuriously.  Point `APEXROUTER_CLI_HOME`
...
fn default_vertex_when_no_model_uses_alias() {
    ...
    // same problem, same fix: redirect APEXROUTER_CLI_HOME so the host global config
