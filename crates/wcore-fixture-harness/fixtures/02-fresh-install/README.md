...
A brand-new `$APEXROUTER_CLI_HOME` with only the minimal config the engine writes on
...
| **B-2** Boot-path order / flag short-circuit | F-012, F-018 | `Config::resolve` runs before flag dispatch; `--list-sessions` creates `~/.apexrouter-cli` files in the real $HOME |
| **B-5** Hermeticity / sandbox leak | F-010, F-035 | Engine reads real `~/Library/Application Support/apexrouter-cli/` during an empty-home boot |

## Scenarios that replay against this fixture (Wave 2+)

1. Boot with `APEXROUTER_CLI_HOME=<this dir>`, `HOME=/tmp/sealed-home` — assert nothing
   written outside `APEXROUTER_CLI_HOME`. Catches B-5.
