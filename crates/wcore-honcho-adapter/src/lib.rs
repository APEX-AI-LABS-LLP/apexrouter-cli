...
//! to `apexrouter-honcho::HonchoClient`.
//!
...
//!
//! ```text
//! APEXROUTER_CLI_USER_MODEL_BACKEND=local   → LocalBackend (default)
//! APEXROUTER_CLI_USER_MODEL_BACKEND=honcho  → HonchoUserModelBackend
//! ```
//!
...
use apexrouter_honcho::HonchoClient;
...
///
/// `APEXROUTER_CLI_USER_MODEL_BACKEND=local` (default) returns an in-memory
...
///
/// `APEXROUTER_CLI_USER_MODEL_BACKEND=honcho` returns a `HonchoUserModelBackend`
...
pub fn select_backend_from_env() -> Result<Arc<dyn UserModelBackend>, UserModelError> {
    let choice =
        std::env::var("APEXROUTER_CLI_USER_MODEL_BACKEND").unwrap_or_else(|_| "local".to_string());
    match choice.as_str() {
        ...
        "honcho" => {
            ...
            if std::env::var("HONCHO_API_KEY").is_err() {
                return Err(UserModelError::Config(
                    "HONCHO_API_KEY not set (required for APEXROUTER_CLI_USER_MODEL_BACKEND=honcho)"
                        .to_string(),
                ));
            }
        ...
        other => Err(UserModelError::Config(format!(
            "unknown APEXROUTER_CLI_USER_MODEL_BACKEND: {other}"
        ))),
    }
}
...
mod tests {
    ...
    async fn brief_merges_dialectic_inferences_from_representations() {
        ...
        let client = Arc::new(HonchoClient::mock());
        client
            .learn_preference("alice", "apexrouter.name", "Alice")
            .await
            .expect("learn_preference must succeed");
        let seeded = vec![
            apexrouter_honcho::DialecticInference {
    ...
    async fn brief_dialectic_is_empty_when_no_representations_seeded() {
        ...
        let client = Arc::new(HonchoClient::mock());
        client
            .learn_preference("bob", "apexrouter.summary", "noted")
            .await
            .unwrap();
        let backend = HonchoUserModelBackend::new(client);
    ...
    fn selector_defaults_to_local() {
        ...
        let prior = std::env::var("APEXROUTER_CLI_USER_MODEL_BACKEND").ok();
        // SAFETY: see comment above.
        unsafe {
            std::env::remove_var("APEXROUTER_CLI_USER_MODEL_BACKEND");
        }
        ...
        unsafe {
            if let Some(v) = prior {
                std::env::set_var("APEXROUTER_CLI_USER_MODEL_BACKEND", v);
            }
        }
    }
    ...
    fn selector_unknown_backend_errors() {
        ...
        let prior = std::env::var("APEXROUTER_CLI_USER_MODEL_BACKEND").ok();
        unsafe {
            std::env::set_var("APEXROUTER_CLI_USER_MODEL_BACKEND", "fictional");
        }
        ...
        unsafe {
            match prior {
                Some(v) => std::env::set_var("APEXROUTER_CLI_USER_MODEL_BACKEND", v),
                None => std::env::remove_var("APEXROUTER_CLI_USER_MODEL_BACKEND"),
            }
        }
    }
    ...
    fn selector_picks_honcho_when_configured() {
        // SAFETY: see selector_defaults_to_local for env-mutation caveat.
        unsafe {
            std::env::set_var("APEXROUTER_CLI_USER_MODEL_BACKEND", "honcho");
        }
