...
IJFW invocation depends on platform: Claude Code uses slash commands (`/ijfw-status`); shell CLIs (Codex, Hermes, ApexRouter, OpenCode, Qwen, Cline, Kimi, OpenClaw, Aider, terminal) use `ijfw status`; Gemini maps intent phrases. See your platform's rules file. IJFW currently targets 14 platforms: Claude Code, Codex, Gemini, Cursor, Windsurf, Copilot, Hermes, ApexRouter, OpenCode, Qwen Code, Cline, Kimi Code, OpenClaw, Aider.
