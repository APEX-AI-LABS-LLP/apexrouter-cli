...
//!
//! Loads `ApexRouterIjfw` via its public `Plugin` trait, builds a
...
//!
//! Providers are deliberately *not* registered (that's apexrouter-ollama's
...
use apexrouter_ijfw::{ApexRouterIjfw, agents, hooks, mcp, rules, skills, tools};
...
#[tokio::test]
async fn apexrouter_ijfw_exercises_every_register_method() {
    let plugin = ApexRouterIjfw;
    ...
    // Providers are deliberately not granted: the apexrouter-ijfw manifest
    ...
    // registration in `apexrouter_ijfw::mcp::register` deliberately skips
    ...
    // Providers — apexrouter-ijfw deliberately leaves this empty (the
    // apexrouter-ollama plugin is the reference provider implementation).
    assert!(
        providers_host.seen.is_empty(),
        "apexrouter-ijfw must not register providers (boundary with apexrouter-ollama)"
    );
}
