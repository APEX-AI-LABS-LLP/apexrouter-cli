...
//! partition not granted by the manifest. The apexrouter-ijfw manifest
//! omits P5; both `read(P5, ..)` and `write(P5, ..)` must fail with
//! `PluginError::PermissionDenied`.

use chrono::Utc;
use apexrouter_ijfw::MANIFEST_TOML;
...
fn p5_read_is_denied_per_manifest() {
    ...
    let client = ScopedMemoryClient::new(&m, &mut host)
        .expect("apexrouter-ijfw manifest grants memory access (P2 readable)");
    ...
    match err {
        PluginError::PermissionDenied { plugin, operation } => {
            assert_eq!(plugin, "apexrouter-ijfw");
            assert!(
...
fn p2_read_is_allowed_per_manifest() {
    ...
    let items = client
        ...
        .expect("P2 read should succeed for apexrouter-ijfw");
