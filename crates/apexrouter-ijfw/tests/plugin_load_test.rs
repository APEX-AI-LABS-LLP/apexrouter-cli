...
use apexrouter_ijfw::{MANIFEST_TOML, ApexRouterIjfw};
...
fn manifest_parses_and_declares_full_surface() {
    ...
    assert_eq!(m.plugin.name, "apexrouter-ijfw");
    assert_eq!(m.plugin.entry.as_deref(), Some("builtin:apexrouter_ijfw"));
    ...
    // Providers are explicitly NOT granted — apexrouter-ollama owns that
