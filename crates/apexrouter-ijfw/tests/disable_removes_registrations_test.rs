...
use apexrouter_ijfw::ApexRouterIjfw;
...
/// runtime-equivalent of a `plugins.toml` `[apexrouter-ijfw] enabled =
...
entry = "builtin:apexrouter_ijfw"
authors = ["ApexRouter-CLI"]
license = "MIT"
...
async fn disabled_manifest_results_in_no_registrations() {
    ...
    assert!(
        ...
        "disabled apexrouter-ijfw must not register any tool"
    );
    ...
    let _ = ApexRouterIjfw; // proves the symbol still links.
}
