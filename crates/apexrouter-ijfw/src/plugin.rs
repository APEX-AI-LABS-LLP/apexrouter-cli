//! Plugin / PluginFactory glue for the apexrouter-ijfw anchor plugin.
//!
//! `ApexRouterIjfwFactory` is submitted via `inventory::submit!` so it's
//! discoverable through the host-side `PluginLoader::discover` path
//! without any explicit registration in main(). `ApexRouterIjfw::initialize`
...
//! (apexrouter-ollama is the provider-only reference plugin).
//!
...
fn manifest() -> &'static PluginManifest {
    ...
    M.get_or_init(|| {
        ...
        PluginManifest::from_toml_str(MANIFEST_TOML)
            .expect("apexrouter-ijfw plugin.toml must parse and validate")
    })
}

pub struct ApexRouterIjfw;

#[async_trait]
impl Plugin for ApexRouterIjfw {
...
pub struct ApexRouterIjfwFactory;

impl PluginFactory for ApexRouterIjfwFactory {
    fn name(&self) -> &'static str {
        "apexrouter-ijfw"
    }

    fn build(&self) -> Box<dyn Plugin> {
        Box::new(ApexRouterIjfw)
    }
}

inventory::submit! { &ApexRouterIjfwFactory as &dyn PluginFactory }
