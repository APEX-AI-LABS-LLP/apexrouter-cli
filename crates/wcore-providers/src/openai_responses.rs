...
//! into apexrouter-cli's provider-neutral [`LlmRequest`] / [`LlmEvent`] types so
...
pub(crate) fn build_responses_body(request: &LlmRequest, _compat: &ProviderCompat) -> Value {
    let mut body = json!({
        ...
        // apexrouter-cli manages its own history. Mirrors OpenClaw's default.
        "store": false,
...
/// or — when it carries tool results (apexrouter-cli threads tool results as
...
/// Map a `response.completed` status to a apexrouter-cli [`StopReason`].
