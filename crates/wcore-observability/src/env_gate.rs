//! D.2 (v0.6.3) — canonical disable-vocabulary for `APEXROUTER_CLI_*` env gates.
//!
//! Round 2 audit found the "mirror" family of opt-out env gates accepted
//! inconsistent disable values: `APEXROUTER_CLI_TRACE_RESULT_SNIPPETS` honoured
//! `off`/`0`/`false`, while `APEXROUTER_CLI_KG` and `APEXROUTER_CLI_MEMORY_STALENESS`
...
//! every `APEXROUTER_CLI_*` opt-out gate accepts the same set. `wcore-observability`
...
/// trimmed). This is the canonical disable vocabulary for `APEXROUTER_CLI_*`
...
/// via APEXROUTER_CLI_*" gate pattern.
...
mod tests {
    ...
    fn enabled_unless_disabled_defaults_on_when_unset() {
        ...
        unsafe {
            std::env::remove_var("APEXROUTER_CLI_TEST_GATE_UNSET");
        }
        assert!(enabled_unless_disabled("APEXROUTER_CLI_TEST_GATE_UNSET"));
    }
    ...
    fn enabled_unless_disabled_honors_every_disable_token() {
        for tok in ["off", "0", "false", "no", "OFF", "False"] {
            ...
            unsafe {
                std::env::set_var("APEXROUTER_CLI_TEST_GATE_OFF", tok);
            }
            assert!(
                !enabled_unless_disabled("APEXROUTER_CLI_TEST_GATE_OFF"),
                "{tok} must disable the gate"
            );
        }
        unsafe {
            std::env::remove_var("APEXROUTER_CLI_TEST_GATE_OFF");
        }
    }
}
