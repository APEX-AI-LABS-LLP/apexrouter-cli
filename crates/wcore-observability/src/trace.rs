...
//! the `APEXROUTER_CLI_TRACE_RESULT_SNIPPETS` env var: capture is ON by default;
...
pub const ENV_RESULT_SNIPPETS: &str = "APEXROUTER_CLI_TRACE_RESULT_SNIPPETS";

/// Returns `true` unless `APEXROUTER_CLI_TRACE_RESULT_SNIPPETS` is set to a
/// recognized disable token (`off`/`0`/`false`/`no`, case-insensitive).
/// D.2 (v0.6.3): routes through [`crate::env_gate::enabled_unless_disabled`]
/// — the canonical `APEXROUTER_CLI_*` disable vocabulary — so this gate and the
...
impl ToolCallTrace {
    ...
    /// `APEXROUTER_CLI_TRACE_RESULT_SNIPPETS` is set to `off`/`0`/`false`, this is
...
mod tests {
    ...
    // ---- W9: APEXROUTER_CLI_TRACE_RESULT_SNIPPETS env gate ----
