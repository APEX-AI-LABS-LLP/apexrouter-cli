//! Plugin / PluginFactory glue for the apexrouter-ollama reference plugin.
//!
//! `ApexRouterOllamaFactory` is submitted via `inventory::submit!` so it's
//! discoverable through the host-side `PluginLoader::discover` path
//! without any explicit registration in main(). `ApexRouterOllama::initialize`
...
fn manifest() -> &'static PluginManifest {
    ...
    M.get_or_init(|| {
        ...
        PluginManifest::from_toml_str(MANIFEST_TOML)
            .expect("apexrouter-ollama plugin.toml must parse and validate")
    })
}

pub struct ApexRouterOllama;

#[async_trait]
impl Plugin for ApexRouterOllama {
...
pub struct ApexRouterOllamaFactory;

impl PluginFactory for ApexRouterOllamaFactory {
    fn name(&self) -> &'static str {
        "apexrouter-ollama"
    }

    fn build(&self) -> Box<dyn Plugin> {
        Box::new(ApexRouterOllama)
    }
}

inventory::submit! { &ApexRouterOllamaFactory as &dyn PluginFactory }
