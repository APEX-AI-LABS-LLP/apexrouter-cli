...
//!
//! Confirms the `ApexRouterOllamaFactory` is submitted to the inventory
...
use apexrouter_ollama::{MANIFEST_TOML, ApexRouterOllama, ApexRouterOllamaFactory};

#[test]
fn plugin_manifest_parses_and_validates() {
    let manifest =
        PluginManifest::from_toml_str(MANIFEST_TOML).expect("manifest must parse + validate");
    assert_eq!(manifest.plugin.name, "apexrouter-ollama");
...
fn factory_name_matches_manifest_name() {
    let factory = ApexRouterOllamaFactory;
    assert_eq!(factory.name(), "apexrouter-ollama");
...
fn plugin_factory_is_discoverable_via_inventory() {
    ...
    // apexrouter-ollama appears exactly once. The discovery path is what
    // `PluginLoader::discover` consumes at runtime.
    let mut count_ollama = 0;
    for factory in inventory::iter::<&'static dyn PluginFactory> {
        if factory.name() == "apexrouter-ollama" {
            count_ollama += 1;
        }
    }
    assert_eq!(
        count_ollama, 1,
        "apexrouter-ollama must be submitted exactly once via inventory::submit!"
    );
}

#[test]
fn apexrouterollama_struct_is_constructible_directly() {
    ...
    let plugin = ApexRouterOllama;
    let m = plugin.manifest();
    assert_eq!(m.plugin.name, "apexrouter-ollama");
}
...
// adapters live in wcore-agent and importing them from apexrouter-ollama
