...
//! contract of the real backends so flipping `APEXROUTER_CLI_SANDBOX=none`
    ...
    WARN_ONCE.call_once(|| {
        tracing::warn!(
            ...
            "sandbox DISABLED — child processes run with host permissions. \
             Install bubblewrap (Linux), or set APEXROUTER_CLI_SANDBOX=docker for opt-in Docker.",
        );
    });
}
...
mod tests {
    ...
    async fn env_is_scrubbed_then_repopulated() {
        ...
        unsafe {
            std::env::set_var("APEXROUTER_CLI_SANDBOX_TEST_LEAK", "leaked");
        }
        let backend = NoSandboxBackend::new();
        ...
        assert!(
            !stdout.contains("APEXROUTER_CLI_SANDBOX_TEST_LEAK"),
