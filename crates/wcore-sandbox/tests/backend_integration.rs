...
//! env-based (`APEXROUTER_CLI_SANDBOX`) selection. The point is to exercise each
//! backend in isolation, not the selection logic, so these tests own their
//! backend choice and do NOT depend on CI setting `APEXROUTER_CLI_SANDBOX=none`
//! (Audit B M2).
//!
...
// OR the host has not opted in via APEXROUTER_CLI_SANDBOX_LIVE_WINDOWS.
//
...
// runner image, group policy unlock, etc.) set `APEXROUTER_CLI_SANDBOX_LIVE_WINDOWS=1`
...
async fn appcontainer_execute_trivial_command_returns_exit_zero() {
    if std::env::var("APEXROUTER_CLI_SANDBOX_LIVE_WINDOWS").is_err() {
        eprintln!(
            "skip: APEXROUTER_CLI_SANDBOX_LIVE_WINDOWS not set \
