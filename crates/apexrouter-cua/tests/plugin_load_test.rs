...
use apexrouter_cua::{MANIFEST_TOML, ApexRouterCua, default_cua_spec};
...
fn manifest_parses_and_declares_register_tools() {
    ...
    assert_eq!(m.plugin.name, "apexrouter-cua");
...
fn plugin_manifest_round_trips_through_factory() {
    let plugin = ApexRouterCua;
    let m = plugin.manifest();
    assert_eq!(m.plugin.entry.as_deref(), Some("builtin:apexrouter_cua"));
}
