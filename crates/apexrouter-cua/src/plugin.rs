//! Plugin / PluginFactory glue for the apexrouter-cua plugin shell.
//!
//! `ApexRouterCuaFactory` is submitted via `inventory::submit!` so it's
//! discoverable through the host-side `PluginLoader::discover` path
//! without any explicit registration in main(). `ApexRouterCua::initialize`
...
//! on restricted ApexRouter compositors at boot time. The plugin layer
...
fn manifest() -> &'static PluginManifest {
    ...
    M.get_or_init(|| {
        ...
        PluginManifest::from_toml_str(MANIFEST_TOML)
            .expect("apexrouter-cua plugin.toml must parse and validate")
    })
}

/// Default `CuaToolSpec` for the apexrouter-cua plugin. The host reads
/// this via `ApexRouterCua::cua_spec()` to construct a real `CuaTool` at
...
impl Plugin for ApexRouterCua {
    ...
    async fn initialize(&self, ctx: &mut PluginContext<'_>) -> PluginResult<()> {
        ...
        // `cua_spec()` — apexrouter-cua cannot construct it directly
        ...
        let registry = ctx.tools.as_mut().ok_or_else(|| {
            wcore_plugin_api::PluginError::HostMisconfiguration {
                plugin: "apexrouter-cua".into(),
                surface: "tools".into(),
            }
        })?;
        // The `execute` tool is a namespace-claim only: the real CuaTool
        // is reified host-side from `cua_spec()` (apexrouter-cua cannot
        ...
        // post-initialize. Mirrors the apexrouter-browser pattern. Without
...
pub struct ApexRouterCuaFactory;

impl PluginFactory for ApexRouterCuaFactory {
    fn name(&self) -> &'static str {
        "apexrouter-cua"
    }

    fn build(&self) -> Box<dyn Plugin> {
        Box::new(ApexRouterCua)
    }
}

inventory::submit! { &ApexRouterCuaFactory as &dyn PluginFactory }
