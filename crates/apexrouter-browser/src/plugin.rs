//! Plugin / PluginFactory glue for the apexrouter-browser plugin shell.
//!
//! `ApexRouterBrowserFactory` is submitted via `inventory::submit!` so it's
//! discoverable through the host-side `PluginLoader::discover` path
//! without any explicit registration in main(). `ApexRouterBrowser::initialize`
...
fn manifest() -> &'static PluginManifest {
    ...
    M.get_or_init(|| {
        ...
        PluginManifest::from_toml_str(MANIFEST_TOML)
            .expect("apexrouter-browser plugin.toml must parse and validate")
    })
}

/// Default `BrowserToolSpec` for the apexrouter-browser plugin. The host
/// reads this via `ApexRouterBrowser::browser_spec()` to construct a real
...
pub struct ApexRouterBrowser;

impl ApexRouterBrowser {
...
#[async_trait]
impl Plugin for ApexRouterBrowser {
...
pub struct ApexRouterBrowserFactory;

impl PluginFactory for ApexRouterBrowserFactory {
    fn name(&self) -> &'static str {
        "apexrouter-browser"
    }

    fn build(&self) -> Box<dyn Plugin> {
        Box::new(ApexRouterBrowser)
    }
}

inventory::submit! { &ApexRouterBrowserFactory as &dyn PluginFactory }
