...
use apexrouter_browser::{MANIFEST_TOML, ApexRouterBrowser, default_browser_spec};
...
fn manifest_parses_and_declares_register_tools() {
    ...
    assert_eq!(m.plugin.name, "apexrouter-browser");
...
fn plugin_manifest_round_trips_through_factory() {
    let plugin = ApexRouterBrowser;
    let m = plugin.manifest();
    assert_eq!(m.plugin.entry.as_deref(), Some("builtin:apexrouter_browser"));
}
