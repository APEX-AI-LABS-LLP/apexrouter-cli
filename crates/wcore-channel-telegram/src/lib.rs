...
mod tests {
    ...
    fn cfg() -> TelegramConfig {
        ...
        // under a pid-unique APEXROUTER_CLI_HOME so concurrent test processes — and
        ...
        ISOLATE.call_once(|| {
            ...
            unsafe { std::env::set_var("APEXROUTER_CLI_HOME", &dir) };
        });
    ...
    async fn longpoll_ingests_message_into_inbox() {
        // State isolation (pid-unique APEXROUTER_CLI_HOME) is set up by `cfg()` below.
        let mut server = mockito::Server::new_async().await;
