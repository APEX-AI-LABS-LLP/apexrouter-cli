...
fn validate_local_path(raw: &str, downloads_root: Option<&Path>) -> Result<PathBuf, String> {
    ...
    // legitimately live under a dotdir like `~/.apexrouter/downloads`); the
...
/// non-traversal `dest_path` like `/etc/cron.d/apexrouter` to slip through.
///
/// Uses `std::env::temp_dir()/apexrouter-downloads` (`dirs` is not a dependency
...
fn default_downloads_root() -> PathBuf {
    let root = std::env::temp_dir().join("apexrouter-downloads");
...
mod tests {
    ...
    /// dotfile, not on the secret deny-list — like `/etc/cron.d/apexrouter` must
    ...
    async fn download_without_explicit_root_rejects_outside_default_root() {
        ...
        let bad_input = json!({
            "op": {
                "kind": "download",
                "url": "https://example.com/x",
                "dest_path": "/etc/cron.d/apexrouter"
            }
        });
