...
//! (`APEXROUTER_CLI_SANDBOX=none`) and assert that every execute path —
...
//!
//! `APEXROUTER_CLI_SANDBOX` is process-global, so every test in this file is
...
///
/// `APEXROUTER_CLI_SANDBOX=none` selects the NoSandbox backend, but the security
...
/// accepts running with no isolation via `APEXROUTER_CLI_ALLOW_NO_SANDBOX=1`. These
