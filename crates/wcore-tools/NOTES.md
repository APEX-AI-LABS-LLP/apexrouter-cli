...
**Source:** `apexrouter-hermes/agent/tools/approval.py` (1158 LOC)

**Decision:** Do not port to `wcore-tools`.

**Overlap with existing ApexRouter infrastructure:**
...
**Source:** `apexrouter-hermes/agent/tools/file_operations.py` (1384 LOC)
...
- `wcore-tools/src/path_validation.rs` and the broader
  ...
  `WRITE_DENIED_PREFIXES` / `APEXROUTER_CLI_WRITE_SAFE_ROOT`. If a future ticket
...
ApexRouter's tool layer operates directly against the local filesystem
...
multiplexer has no home in ApexRouter's architecture.

## T3-3.2.5 file_tools — NO-LIFT

**Source:** `apexrouter-hermes/agent/tools/file_tools.py` (810 LOC)

**Decision:** Do not port to `wcore-tools`. Complete overlap with the
existing ApexRouter built-in tool surface — every public tool in
`file_tools.py` already has a Rust equivalent.

**Tool-by-tool overlap mapping:**

| Hermes (`file_tools.py`)            | ApexRouter built-in                       |
|-------------------------------------|----------------------------------------|
...
- `_is_blocked_device` (L85) — `/dev/zero`, `/dev/random`, etc. blocklist.
  ApexRouter's `read.rs` + `path_validation` handle device-path safety via
...
- `_get_max_read_chars` / `_DEFAULT_MAX_READ_CHARS` (L28-63) — read-size
  guard via `file_read_max_chars` config. ApexRouter's `read.rs` already
...
- `ShellFileOperations` per-task cache (L163, L284) — Hermes shells out
  to `/bin/cat`, `/bin/grep`, etc. through a task-keyed wrapper. ApexRouter
...
- `notify_other_tool_call` / `reset_file_dedup` (L465-501) — Hermes
  dedup state for repeated reads. ApexRouter's `FileStateCache` is the
...
plus task-cached shell wrappers that don't translate to ApexRouter's
direct-fs model. Porting would duplicate every tool in `wcore-tools`.

## T3-3.2.6 path_security — NO-LIFT

**Source:** `apexrouter-hermes/agent/tools/path_security.py` (43 LOC)
...
**Conclusion:** No source change. ApexRouter's path-validation surface is
...
**Source:** `apexrouter-hermes/agent/tools/ansi_strip.py` (44 LOC)
...
fast-path `_HAS_ESCAPE` pre-check. The current ApexRouter regex handles
...
**Source:** `apexrouter-hermes/agent/tools/permission_classifier.py`
...
1. The classifier is a *policy* layer, not a *tool*. Its output
   ...
   + `PermissionMode` from `APEXROUTER_CLI_PERMISSION_MODE`) drives whether
...
3. The `PermissionMode` env-var contract (AGENT_ENV_CONTRACT v1:
   `APEXROUTER_CLI_PERMISSION_MODE`, `APEXROUTER_CLI_PRE_APPROVED_HASHES`,
   `APEXROUTER_CLI_HARDFLOOR_OVERRIDE`, `APEXROUTER_CLI_WORKSPACE_DIR`) needs a
   matching ApexRouter host-side contract before the engine can honor
...
**Source:** `apexrouter-hermes/agent/tools/registry.py` (482 LOC).
...
functionally the same surface as ApexRouter's existing
...
**Overlap with existing ApexRouter infrastructure (1:1 mapping):**

| Hermes (`registry.py`) | ApexRouter equivalent | Notes |
|---|---|---|
| `ToolRegistry.register(name, toolset, schema, handler, …)` | `ToolRegistry::register(Box<dyn Tool>)` | ApexRouter uses trait-object polymorphism; schema lives on `Tool::input_schema()`, handler on `Tool::execute()`. |
| `ToolEntry.toolset` (string grouping) | `Tool::category() -> ToolCategory` (enum) + plugin-api `Toolset` boundary | ApexRouter uses a typed enum at the tool surface and a plugin-isolation boundary above it — strictly stronger than free-form strings. |
| `register_toolset_alias()` / `get_toolset_alias_target()` | n/a — not needed | Aliases are a Python-side artifact for human-typed CLI flags; ApexRouter's CLI doesn't expose toolset names as a user-typed dimension. |
| `discover_builtin_tools()` (AST scan + `importlib`) | Static `register(Box::new(...))` calls in `wcore-agent`/plugin init | ApexRouter resolves discovery at compile time via the plugin system — strictly stronger than runtime AST inspection. |
| MCP refresh via `deregister()` + re-register | `wcore-mcp` client-driven registration into `ToolRegistry` | ApexRouter's MCP layer already owns the refresh contract; replicating Hermes' mutate-during-read locking would duplicate `wcore-mcp` responsibilities. |
| `get_definitions(tool_names, quiet)` → OpenAI `{type, function}` | `ToolRegistry::to_tool_defs()` / `to_tool_defs_filtered(...)` → `ToolDef` | ApexRouter emits provider-neutral `ToolDef` and lets each `LlmProvider` format-convert in `build_request_body()` (per AGENTS.md "no hardcoded provider quirks"). Porting Hermes' OpenAI-shape emitter would re-introduce the quirk. |
| `dispatch(name, args, **kwargs)` (catches all exceptions, returns JSON) | `ToolRegistry::dispatch(tool, input)` / `dispatch_with_ctx(...)` via `ToolDispatcher` trait | ApexRouter adds a per-tool `CircuitBreaker` around dispatch (3 failures / 30s trip) — no counterpart in Hermes. Porting would lose this without merge surgery. |
| `get_max_result_size(name)` | budget config in `wcore-config` / per-tool result truncation in tool impls | ApexRouter keeps result-size policy out of the registry; lifting it would entangle the registry with budget policy. |
| `get_emoji(name, default)` | n/a — ApexRouter surface doesn't carry presentation metadata | Emoji is a UI concern routed through `wcore-protocol` events, not the tool registry. |
...
2. The Python-idiom features that *don't* map cleanly — `emoji`,
   ...
   engine because ApexRouter already routes those concerns elsewhere
...
3. Hermes' module-import-time auto-registration model is a
   ...
   equivalent of "import a `.py` file to register a tool"; ApexRouter's
   plugin-anchor crates (`apexrouter-ijfw` et al. in AGENTS.md crate map)
   already provide the explicit equivalent.

**Conclusion:** No source change in T3-3.4. The ApexRouter `ToolRegistry`
...
**Source:** `apexrouter-hermes/agent/tools/credential_files.py` (422 LOC)
...
a credential store. ApexRouter has no consumer for it, and the actual
...
|----------------------------------------|-------------------------------------------------------------------|
| `register_credential_file` (L70)       | Append a `APEXROUTER_CLI_HOME`-relative path to a `ContextVar`-scoped registry of files to mount into a remote sandbox at `/root/.apexrouter/...`. |
...
**Why this has no ApexRouter home:**

1. **No remote-terminal backend abstraction.** ApexRouter tools
   ...
   "ApexRouter's tool layer operates directly against the local
...
3. **`required_credential_files` skill front matter has no ApexRouter
   counterpart in this wave.** ApexRouter's `wcore-skills` (see
...
4. **Cache-directory mounts (uploads, screenshots, TTS audio, images)
   are gateway-side concerns.** ApexRouter's browser stack
...
- The `ContextVar`-scoped session registry (L33-L43) — a Python
  ...
  pipeline. ApexRouter's per-session state lives on `ToolContext` /
  `SessionState`, not in module-level context vars; even if a mount
  registry were needed, the Python pattern would not translate.

**Cross-tier follow-up (filed, not done here):**

- If a future ApexRouter wave introduces a remote-sandbox terminal
  backend (Docker / Modal / SSH analogue), this is where the
  file-mount registry would live — but as a `apexrouter-remote-backend`
...
that does not exist in ApexRouter's architecture. The credential-storage
...
**Source:** `apexrouter-hermes/agent/tools/mcp_oauth.py` (482 LOC)
...
3. **The Hermes module is a thin SDK adapter, not an OAuth
   ...
   only contributes:
   - `ApexRouterTokenStorage` — on-disk token/client-info persistence
     under `APEXROUTER_CLI_HOME/mcp-tokens/<server>.json` with 0o600
...
- The storage layout (`APEXROUTER_CLI_HOME/mcp-tokens/<safe-name>.json` and
...
- The browser-detection ladder (SSH_CLIENT / SSH_TTY → no;
  Windows / macOS → yes; Linux/X11 / ApexRouter → only if `DISPLAY`
  or `APEXROUTER_CLI_DISPLAY`). This determines whether we attempt
...
**Source:** `apexrouter-hermes/agent/tools/process_registry.py` (1211 LOC).
...
2. **~70% of hermes LOC wires to absent subsystems** —
   ...
   `~/.apexrouter/processes.json`. None of those concerns have a ApexRouter
   equivalent.
...
dead weight for ApexRouter's architecture.
...
**Source:** `apexrouter-hermes/agent/tools/browser_camofox_state.py` (47 LOC).
Exposes two helpers:

- `get_camofox_state_dir() -> Path` returns
  `APEXROUTER_CLI_HOME/browser_auth/camofox` — the host-side root that the
  Camofox sidecar persists profile data under.
- `get_camofox_identity(task_id) -> {user_id, session_key}` derives a
  profile-stable `apexrouter_<10hex>` user id (uuid5 over the state-dir
  path) and a per-task `task_<16hex>` session key (uuid5 over
  `<state-dir>:<task_id>`). The ApexRouter gateway sends these to the
...
- `crates/wcore-browser/src/backends/camoufox.rs:117-148` sends only
  ...
  `user_id` / `session_key` field on the wire, and the sidecar ApexRouter
...
- A separate user-id/session-key identity contract requires (a) a
  matching field on the Camofox HTTP API, (b) a ApexRouter-side notion of
...
- The directory layout under `<apexrouter-home>/browser_auth/camofox/` so
  operators inheriting existing persistent profiles do not lose them.
  `apexrouter_home` resolution would route through `wcore-config` (the
  central `APEXROUTER_CLI_HOME` resolver), not a free helper.
- The uuid5 derivation strategy (`NAMESPACE_URL`,
  ...
  profile root yields the same `apexrouter_<...>` / `task_<...>` strings
...
- When `wcore-browser`'s Camofox backend gains a ApexRouter-managed
  identity field on `open_session`, the natural home is
  `crates/wcore-browser/src/backends/camoufox.rs` (alongside the
  existing `persistent_profile` plumbing) with the `APEXROUTER_CLI_HOME`
...
Camofox HTTP surface grows a ApexRouter identity field; until then, the
helpers stay in Hermes.

## T3-3.4 checkpoint_manager — NO-LIFT (orchestrator-applied)

**Source:** `apexrouter-hermes/agent/tools/checkpoint_manager.py` (635 LOC).
...
`~/.apexrouter/checkpoints/{sha256(abs_dir)[:16]}/` driven by
...
**Source:** `apexrouter-hermes/agent/tools/browser_tool.py` (2596 LOC,
...
| `_get_vision_model` / `_get_extraction_model` | Provider-internal model selection (config-cascade in ApexRouter) |
...
(`apexrouter-browser`) goes through `wcore-plugin-api` per audit F2 with
...
**Source:** `apexrouter-hermes/agent/tools/browser_cdp_tool.py` (564 LOC)
...
- `_resolve_cdp_endpoint` delegates to `tools.browser_tool._get_cdp_override`,
  ...
  key set by Hermes's `/browser connect` slash command. The ApexRouter
  CLI has no equivalent `connect` command surface, and the cdp_url
  config field is not part of the ApexRouter config schema.
...
**Source:** `apexrouter-hermes/agent/tools/browser_dialog_tool.py` (148 LOC).
...
3. Plugin-mirror: `apexrouter-browser` would also need a
...
the LLM via `browser_snapshot.pending_dialogs[]`. ApexRouter's
...
**Source:** `apexrouter-hermes/agent/tools/browser_camofox.py` (613 LOC).
...
ApexRouter and is READ-ONLY in this sub-wave.

**Existing ApexRouter implementation:**
...
**Hermes → ApexRouter surface mapping:**

| Hermes (`browser_camofox.py`)             | ApexRouter equivalent                                  |
|-------------------------------------------|-----------------------------------------------------|
...
3. **The Hermes module is a tool-surface façade, not a backend.**
   ...
   sidecar HTTP calls ApexRouter's `CamoufoxBackend` already wraps. In
   ApexRouter, the equivalent agent-side surface is `BrowserTool`
   (defined per the `apexrouter-browser` plugin crate per `AGENTS.md`)
...
4. **The `ContextVar`-scoped per-task session map** (Hermes lines 139,
   ...
   the gateway request pipeline. ApexRouter's `BrowserSession` /
...
5. **`camofox_vision` (Hermes line 506) is a model-side concern, not
   ...
   into an LLM call. The ApexRouter equivalent flows through
...
- `_safe_skills_path` / `_safe_filename`-style sanitizers — Hermes
  ...
  enforces ref scoping. ApexRouter's policy gate is `BrowserPolicy` and
  the request signing happens at the reqwest layer.
- `get_vnc_url()` — VNC is an operator-debug surface exposed by the
  sidecar's `/health` endpoint. ApexRouter's host-integration story is
...
- The `_managed_persistence_enabled()` env-driven storage-mode flag —
  ...
  concern. The ApexRouter operator configures the sidecar directly.

**Cross-tier follow-ups (filed, not done here):**

- If a future ApexRouter wave decides to surface VNC URLs as part of an
...
**Source:** `apexrouter-hermes/agent/tools/browser_supervisor.py` (1362 LOC)
...
| Public API | `respond_to_dialog`, snapshot read, dialog-bridge XHR endpoint (`apexrouter-dialog-bridge.invalid`) | `register`, `on_session_end`, `live_sessions`, `start_reaper`, `healthcheck`, `launch_camoufox` |
...
3. **No stub-and-orphan port.** The CDP-supervisor concern requires:
   ...
   - injected `addScriptToEvaluateOnNewDocument` dialog bridge + Fetch
     URL-pattern interception of `apexrouter-dialog-bridge.invalid`
...
- Surfaces a `Snapshot { dialogs, frame_tree, recent_console }`
  ...
  return path inside the apexrouter-browser plugin's
...
**Source:** `apexrouter-hermes/agent/tools/openrouter_client.py` (44 LOC)
...
**Source:** `apexrouter-hermes/agent/tools/xai_http.py` (12 LOC)
...
Single function `apexrouter_xai_user_agent() -> str` returning
`f"ApexRouter-Agent/{__version__}"` — a User-Agent string for direct xAI
HTTP calls.
...
3. **`wcore-providers/*` is READ-ONLY** this sub-wave; xAI provider is
   Tier-1 scope.
4. **Trivial Rust equivalent**: a one-line `format!("ApexRouter-Agent/{}",
...
multiple providers need a ApexRouter-branded UA, extract to
...
**Source:** `apexrouter-hermes/agent/tools/code_execution_tool.py` (1416 LOC).
...
**Why NO-LIFT — ApexRouter already covers this:**

| Hermes capability | ApexRouter surface |
|---|---|
...
ApexRouter's ScriptTool is **strictly safer**: refuses arbitrary Python in
...
sandbox shipping has no ApexRouter analog or roadmap pull. Same pattern as
T3-3.2.4 file_operations NO-LIFT.
...
**Source:** `apexrouter-hermes/agent/tools/terminal_tool.py` (1717 LOC).
...
ApexRouter has zero of those backend layers. `crates/wcore-tools/src/bash.rs`
...
- Multi-backend (docker/modal/ssh/etc.) — no ApexRouter equivalent of the
  `tools.environments.local` abstraction.

**Future-hook sketch:**

If a remote execution backend is ever added (e.g. `apexrouter-remote-backend`
...
of scope for ApexRouter's architecture.

## T3-3.7 team_invoke / team_list — NO-LIFT

**Source:** `apexrouter-hermes/agent/tools/team_invoke.py` (351 LOC).

**What the hermes module does:**

Two model-facing tools (`team_invoke`, `team_list`) registered into the
`apexrouter_teams` toolset. Both are thin JSON adapters over a separate
...
3. `apexrouter_cli.agent_registry` — v2 AgentRegistry for resolving
...
isolated subagent children) and ApexRouter mirrors that as
...
When ApexRouter eventually grows a TEAMS subsystem, the right cuts are:
...
4. Plugin authors register panels via `wcore-plugin-api` extension
   points (similar to how `apexrouter-ijfw` exercises every `register_*`
   surface).
...
**Source:** `apexrouter-hermes/agent/tools/kanban_tools.py` (876 LOC). Sub-
...
shim — 100% of its semantics delegate to `apexrouter_cli.kanban_db`, a
SQLite-backed dispatcher subsystem that does not exist in apexrouter-cli
...
- Dispatcher process model (`APEXROUTER_CLI_KANBAN_TASK`, `APEXROUTER_CLI_KANBAN_RUN_ID`,
  `APEXROUTER_CLI_KANBAN_CLAIM_LOCK` env contract).
- Profile-config integration (`load_config().toolsets`) for orchestrator gating.
- Multi-tenant scoping (`APEXROUTER_CLI_TENANT`).
...
the kanban subsystem is ever scoped for apexrouter-cli.
...
**Source:** `apexrouter-hermes/agent/tools/memory_tool.py` (666 LOC).
...
The ApexRouter `wcore-memory` crate is categorically richer:
...
**Why NO-LIFT — ApexRouter already covers this through a categorically
richer architecture:**

1. **Intent overlap, shape mismatch.** Hermes `add/replace/remove`
   over a flat MD file targets the same intent as ApexRouter
   ...
   of ApexRouter would regress the data model.
...
3. **Frozen-snapshot pattern already lives in `wcore-memory`.** The
   ...
   apexrouter-cli equivalent of Hermes's `format_for_system_prompt` +
...
5. **Same pattern as prior NO-LIFTs.** This mirrors the rationale in
   ...
   ApexRouter's richer Tier-1/Tier-2 architecture obsoletes the Hermes
...
**What is preserved by the existing ApexRouter stack:**
...
**Source:** `apexrouter-hermes/agent/tools/skill_manager_tool.py` (799 LOC).

**Decision:** Do not port. `wcore-skills` already owns the entire
skill lifecycle that this tool wraps. The hermes file is an
agent-facing CRUD shim over `~/.apexrouter/skills/` (create / edit /
...
|---|---|
| Resolve `~/.apexrouter/skills/` | `paths::user_skills_dir()` |
| Project-level `.apexrouter-cli/skills/` discovery | `paths::project_skills_dirs()`, `discovery::RuntimeDiscovery` |
...
**Source:** `apexrouter-hermes/agent/tools/skills_tool.py` (1436 LOC).
...
platforms gating / progressive disclosure tiers / `~/.apexrouter/skills/`
discovery.
...
**Source:** `apexrouter-hermes/agent/tools/skill_provenance.py` (78 LOC).
...
3. **`ContextVar` pattern explicitly rejected for this codebase.**
   T3-3.4 `env_passthrough` documented: *"ApexRouter runs as a single-
