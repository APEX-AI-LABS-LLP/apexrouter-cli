//! T3-3.8 — ApexRouter self-introspection toolset (`apexrouter_status` +
//! `apexrouter_telemetry_query`).
//!
//! Ported from `apexrouter-hermes/agent/tools/apexrouter_introspection.py`
//! (PRD-1 §4.2 + §4.3). The Python original registers two read-only
//! tools as a single `apexrouter_introspection` toolset so the model can
...
//!
//! The Python handlers import `agent.status.apexrouter_status` and
//! `agent.telemetry.apexrouter_telemetry_query` at dispatch time — those
...
//! exposes a [`ApexRouterIntrospectionBackend`] trait the host wires to a
...
//!
//! The Python `_apexrouter_telemetry_query_handler` accepts EITHER a
...
pub const APEXROUTER_CLI_INTROSPECTION_TOOLSET: &str = "apexrouter_introspection";

/// Tool name for the runtime-snapshot tool.
pub const APEXROUTER_CLI_STATUS_TOOL_NAME: &str = "apexrouter_status";

/// Tool name for the telemetry-query tool.
pub const APEXROUTER_CLI_TELEMETRY_QUERY_TOOL_NAME: &str = "apexrouter_telemetry_query";
...
/// Parsed input for the `apexrouter_status` tool.
...
/// Parsed input for the `apexrouter_telemetry_query` tool. Mirrors the
/// Python `_apexrouter_telemetry_query_handler`: `filter` precedence over
...
/// construction time.
#[async_trait]
pub trait ApexRouterIntrospectionBackend: Send + Sync {
...
pub struct NullApexRouterIntrospectionBackend;

#[async_trait]
impl ApexRouterIntrospectionBackend for NullApexRouterIntrospectionBackend {
    async fn status(&self, _query: &StatusQuery) -> IntrospectionOutcome {
        IntrospectionOutcome::Err {
            message: "No apexrouter_introspection backend configured for apexrouter_status. \
                      Wire a ApexRouterIntrospectionBackend implementation when constructing \
                      the tools."
                .to_string(),
        }
    }
    ...
    async fn telemetry_query(&self, _query: &TelemetryQuery) -> IntrospectionOutcome {
        IntrospectionOutcome::Err {
            message: "No apexrouter_introspection backend configured for \
                      apexrouter_telemetry_query. Wire a ApexRouterIntrospectionBackend \
...
#[async_trait]
impl ApexRouterIntrospectionBackend for CapturingIntrospectionBackend {
    async fn status(&self, query: &StatusQuery) -> IntrospectionOutcome {
...
fn status_description() -> &'static str {
    "Return a JSON snapshot of the ApexRouter runtime — model, profile, \
...
fn telemetry_description() -> &'static str {
    "Query the ApexRouter structured telemetry event log (PRD-1 §4.2). \
...
/// `apexrouter_status` tool — single-call snapshot of the ApexRouter runtime.
pub struct ApexRouterStatusTool {
    backend: Arc<dyn ApexRouterIntrospectionBackend>,
    /// v0.9.0 W1: defaults `false` so `Tool::is_available()` hides the
    /// tool when no real backend is wired. `new(backend)` flips it on.
    backend_configured: bool,
}

impl Default for ApexRouterStatusTool {
    fn default() -> Self {
        Self {
            backend: Arc::new(NullApexRouterIntrospectionBackend),
            backend_configured: false,
        }
    }
}

impl ApexRouterStatusTool {
    pub fn new(backend: Arc<dyn ApexRouterIntrospectionBackend>) -> Self {
        Self {
            backend,
...
#[async_trait]
impl Tool for ApexRouterStatusTool {
    fn name(&self) -> &str {
        APEXROUTER_CLI_STATUS_TOOL_NAME
    }

    /// v0.9.0 W1: hidden when no real `ApexRouterIntrospectionBackend` is
...
/// `apexrouter_telemetry_query` tool — structured event-log query.
pub struct ApexRouterTelemetryQueryTool {
    backend: Arc<dyn ApexRouterIntrospectionBackend>,
    /// v0.9.0 W1: defaults `false` so `Tool::is_available()` hides the
    /// tool when no real backend is wired. `new(backend)` flips it on.
    backend_configured: bool,
...
impl Default for ApexRouterTelemetryQueryTool {
    fn default() -> Self {
        Self {
            backend: Arc::new(NullApexRouterIntrospectionBackend),
            backend_configured: false,
...
impl ApexRouterTelemetryQueryTool {
    pub fn new(backend: Arc<dyn ApexRouterIntrospectionBackend>) -> Self {
        Self {
            backend,
            backend_configured: true,
...
#[async_trait]
impl Tool for ApexRouterTelemetryQueryTool {
    fn name(&self) -> &str {
        APEXROUTER_CLI_TELEMETRY_QUERY_TOOL_NAME
    }
...
pub fn build_toolset(
    backend: Arc<dyn ApexRouterIntrospectionBackend>,
) -> (ApexRouterStatusTool, ApexRouterTelemetryQueryTool) {
    (
        ApexRouterStatusTool::new(Arc::clone(&backend)),
        ApexRouterTelemetryQueryTool::new(backend),
    )
}
...
mod tests {
    ...
    fn status_records_session_id_and_returns_payload() {
        ...
        let tool = ApexRouterStatusTool::new(backend.clone());
    ...
    fn null_backend_status_fails_loud_no_silent_stub() {
        let tool = ApexRouterStatusTool::default();
        let res = run(&tool, json!({}));
        assert!(res.is_error);
        assert!(
            res.content
                .contains("No apexrouter_introspection backend configured"),
            "expected fail-loud, got: {}",
            res.content
        );
        assert!(res.content.contains("apexrouter_status"));
    }

    #[test]
    fn null_backend_telemetry_fails_loud_no_silent_stub() {
        let tool = ApexRouterTelemetryQueryTool::default();
        let res = run(&tool, json!({}));
        assert!(res.is_error);
        assert!(
            res.content
        ...
        assert!(res.content.contains("apexrouter_telemetry_query"));
    }

    #[test]
    fn telemetry_defaults_match_python_handler() {
        let backend = Arc::new(CapturingIntrospectionBackend::new());
        let tool = ApexRouterTelemetryQueryTool::new(backend.clone());
        let res = run(&tool, json!({}));
        assert!(!res.is_error);
        let snap = backend.snapshot();
    ...
    fn registry_metadata_matches_python_toolset_unit() {
        ...
        // `apexrouter_introspection` flip both off together.
        assert_eq!(APEXROUTER_CLI_INTROSPECTION_TOOLSET, "apexrouter_introspection");
        assert_eq!(APEXROUTER_CLI_STATUS_TOOL_NAME, "apexrouter_status");
        assert_eq!(APEXROUTER_CLI_TELEMETRY_QUERY_TOOL_NAME, "apexrouter_telemetry_query");

        let backend = Arc::new(CapturingIntrospectionBackend::new());
        let (status, tq) = build_toolset(backend);
        assert_eq!(status.name(), APEXROUTER_CLI_STATUS_TOOL_NAME);
        assert_eq!(tq.name(), APEXROUTER_CLI_TELEMETRY_QUERY_TOOL_NAME);
    ...
    fn status_with_real_backend_is_available() {
        let tool = ApexRouterStatusTool::new(Arc::new(CapturingIntrospectionBackend::new()));
        assert!(
            tool.is_available(),
    ...
    fn telemetry_with_real_backend_is_available() {
        let tool = ApexRouterTelemetryQueryTool::new(Arc::new(CapturingIntrospectionBackend::new()));
        assert!(
            tool.is_available(),
            "new(backend) must yield backend_configured == true"
        );
