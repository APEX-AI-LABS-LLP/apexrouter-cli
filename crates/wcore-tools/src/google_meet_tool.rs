...
//!
//! Ported from `apexrouter-hermes/agent/tools/google_meet_tool.py` (754 LOC).
//!
...
//! BlackHole on macOS). ApexRouter's engine deliberately does **not** embed
//! a browser-automation stack, a per-meeting state directory under
//! `$APEXROUTER_CLI_HOME/workspace/meetings/`, or a virtual-mic bridge — these
//! belong to the host (`wcore-browser` / `apexrouter-browser` / a wcore-cua
...
//!   under `$APEXROUTER_CLI_HOME/workspace/meetings/`, and polls `kill(pid, 0)`
...
/// Python `'ApexRouter Agent'`.
pub const DEFAULT_GUEST_NAME: &str = "ApexRouter Agent";
    ...
    fn input_schema(&self) -> JsonSchema {
        json!({
            ...
            "properties": {
                ...
                "guest_name": {
                    "type": "string",
                    "description": "Display name to use when joining as guest. Defaults to 'ApexRouter Agent'."
                },
                "duration": {
                    "type": "string",
