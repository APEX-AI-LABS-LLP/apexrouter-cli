...
pub fn validate_user_path(path: &Path) -> Result<PathBuf, PathValidationError> {
    ...
    // `notes.txt -> $APEXROUTER_CLI_HOME/cron/jobs.json` (or `~/.ssh/id_rsa`) before
...
fn is_denied_system_path(path: &Path) -> bool {
    ...
    const DENIED_SUFFIXES: &[&str] = &[
        ...
        "/.kube/config",
        // F-054: ApexRouter-CLI own credential files — a prompt-injected agent
        // must not be able to Read the engine's stored secrets back to the model.
        "/.config/apexrouter-cli/credentials.toml",
        "/.apexrouter/credentials.toml",
        "/apexrouter-cli/auth.json",
        "/apexrouter-cli/credentials.enc",
        "/apexrouter-cli/credentials.key.json",
        // M-19: cron state directory (`~/.apexrouter/cron/` — `jobs.json` +
    ...
    // M-19 (residual bypass): the `/.apexrouter/cron/` suffix above only matches
    // the DEFAULT cron dir. The cron store resolves `$APEXROUTER_CLI_HOME` first
...
/// it: `$APEXROUTER_CLI_HOME/cron` when set, else `~/.apexrouter/cron`. Mirrors
...
/// `APEXROUTER_CLI_HOME` is caught whichever way the agent spells the target: a
...
fn resolved_cron_dirs() -> Vec<PathBuf> {
    let Some(home) = std::env::var_os("APEXROUTER_CLI_HOME")
        .map(PathBuf::from)
...
mod tests {
    ...
    // F-054: ApexRouter-CLI own credential files must be blocked.
    #[cfg(unix)]
    #[test]
    fn apexrouter_cli_credentials_toml_rejected() {
        let err = validate_user_path(Path::new(
            "/home/alice/.config/apexrouter-cli/credentials.toml",
        ))
        .unwrap_err();
        assert!(matches!(err, PathValidationError::SystemPath(_)));
    }
    ...
    #[cfg(unix)]
    #[test]
    fn apexrouter_credentials_toml_rejected() {
        let err =
            validate_user_path(Path::new("/home/alice/.apexrouter/credentials.toml")).unwrap_err();
        assert!(matches!(err, PathValidationError::SystemPath(_)));
    }
    ...
    #[cfg(unix)]
    #[test]
    fn apexrouter_cron_jobs_json_rejected() {
        let err = validate_user_path(Path::new("/home/alice/.apexrouter/cron/jobs.json")).unwrap_err();
        assert!(matches!(err, PathValidationError::SystemPath(_)));
    }

    #[cfg(unix)]
    #[test]
    fn apexrouter_cron_integrity_key_rejected() {
        let err =
            validate_user_path(Path::new("/home/alice/.apexrouter/cron/.integrity.key")).unwrap_err();
        assert!(matches!(err, PathValidationError::SystemPath(_)));
    }

    // M-19 (residual bypass): with APEXROUTER_CLI_HOME relocated, the cron store no
    // longer lives under `~/.apexrouter/cron`, so the hardcoded substring missed
    // it. The deny-list must derive the cron dir from the same env the store
    // reads. The literal `/home/alice/.apexrouter/...` tests above prove the
    // default path stays denied regardless of this env var.
    #[cfg(unix)]
    #[test]
    fn apexrouter_cron_relocated_home_jobs_and_key_rejected() {
        // SAFETY: single-threaded test setup; no other test mutates this var.
        unsafe { std::env::set_var("APEXROUTER_CLI_HOME", "/srv/wl-relocated-test") };
        ...
        unsafe { std::env::remove_var("APEXROUTER_CLI_HOME") };
        assert!(
    ...
    // M-19 (residual of the residual): a symlinked APEXROUTER_CLI_HOME let a write to
    ...
    #[cfg(unix)]
    #[test]
    fn apexrouter_cron_symlinked_home_canonical_write_rejected() {
        ...
        unsafe { std::env::set_var("APEXROUTER_CLI_HOME", &link) };
        ...
        assert!(
            ...
            "write to the canonical cron dir under a symlinked APEXROUTER_CLI_HOME must be denied, got {res:?}"
        );
    }
    ...
    #[cfg(unix)]
    #[test]
    fn apexrouter_auth_json_rejected() {
        let err = validate_user_path(Path::new("/home/alice/.config/apexrouter-cli/auth.json"))
            .unwrap_err();
        assert!(matches!(err, PathValidationError::SystemPath(_)));
    }
