...
//!
//! Port of `agent/tools/file_state.py` from apexrouter-hermes. Prevents
...
//! `APEXROUTER_CLI_DISABLE_FILE_STATE_GUARD=1` is set):
//!
...
fn is_disabled() -> bool {
    std::env::var("APEXROUTER_CLI_DISABLE_FILE_STATE_GUARD")
        .map(|v| v.trim() == "1")
        .unwrap_or(false)
}
...
mod tests {
    ...
    impl TestEnv {
        fn new() -> Self {
            ...
            unsafe {
                std::env::remove_var("APEXROUTER_CLI_DISABLE_FILE_STATE_GUARD");
            }
            registry().clear();
            Self { _serial: guard }
        }
    ...
    fn disable_flag_makes_all_ops_noop() {
        ...
        unsafe {
            std::env::set_var("APEXROUTER_CLI_DISABLE_FILE_STATE_GUARD", "1");
        }
