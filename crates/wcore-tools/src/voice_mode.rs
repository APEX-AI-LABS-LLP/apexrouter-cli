...
//!
//! Ported from `apexrouter-hermes/agent/tools/voice_mode.py` (1017 LOC, MIT).
//!
...
impl VoiceMode {
    ...
    /// default temp dir under `std::env::temp_dir()/apexrouter_voice`.
    pub fn new(
        recorder: Arc<dyn AudioRecorder>,
        transcriber: Arc<dyn TranscriptionBackend>,
        player: Arc<dyn AudioPlayer>,
        ...
        Self {
            ...
            player,
            env_probe,
            temp_dir: std::env::temp_dir().join("apexrouter_voice"),
        }
    }
...
mod tests {
    ...
    async fn voice_mode_full_cycle_records_then_transcribes_then_plays() {
        ...
        let wav = PathBuf::from("/tmp/apexrouter_voice_test.wav");
    ...
    fn cleanup_on_missing_dir_returns_zero() {
        let nonexistent = std::env::temp_dir().join("apexrouter_voice_does_not_exist_xyzzy");
    ...
    async fn voice_mode_tool_explicit_stop_surfaces_wav_path() {
        ...
        rec.set_next_outcome(RecordingOutcome::Captured {
            wav_path: PathBuf::from("/tmp/apexrouter_voice_test_b10.wav"),
        });
        ...
        assert!(!r.is_error);
        assert!(
            r.content.contains("apexrouter_voice_test_b10.wav"),
