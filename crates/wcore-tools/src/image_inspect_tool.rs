...
mod tests {
    ...
    /// single ASCII `Make` tag ("ApexRouter"). Hand-assembled because the
    ...
    fn jpeg_with_exif() -> Vec<u8> {
        ...
        // Entry: Make (0x010F), type ASCII (2), count 8 ("ApexRouter\0").
        ...
        tiff.extend_from_slice(b"ApexRouter\0"); // the ASCII value, 8 bytes
    ...
    async fn reports_exif_when_present() {
        ...
        assert!(
            result.content.contains("Make") && result.content.contains("ApexRouter"),
            "expected the embedded Make=ApexRouter tag in: {}",
            result.content
        );
    }
