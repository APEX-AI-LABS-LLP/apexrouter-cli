...
//!
//! Ported from `apexrouter-hermes/agent/tools/gitlab_tool.py` and modeled
//! on `discord_tool.rs`. The Python original talks to GitLab's REST API
//! directly (an HTTP client + a `PRIVATE-TOKEN` header). ApexRouter's
...
/// `gitlab_api` tool — ApexRouter engine port of `gitlab_tool.py`.
