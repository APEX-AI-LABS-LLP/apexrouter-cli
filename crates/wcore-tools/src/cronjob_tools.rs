...
//!
//! Ported from `apexrouter-hermes/agent/tools/cronjob_tools.py`. The Python
...
//! * Origin auto-detection from `APEXROUTER_CLI_SESSION_*` env vars is moved to
...
//!   defers to the host because `~/.apexrouter/scripts/` is a host-defined
//!   directory.
...
pub fn validate_cron_script_path(script: &str) -> Option<String> {
    ...
    if raw.starts_with('/') || raw.starts_with('~') {
        return Some(format!(
            "Script path must be relative to ~/.apexrouter/scripts/. \
             Got absolute or home-relative path: {raw:?}. \
             Place scripts in ~/.apexrouter/scripts/ and use just the filename."
        ));
    }
...
/// `cronjob` tool — ApexRouter engine port of `cronjob_tools.py`.
    ...
    fn input_schema(&self) -> JsonSchema {
        json!({
            ...
            "properties": {
                ...
                "script": {
                    "type": "string",
                    "description": "Optional Python script path under ~/.apexrouter/scripts/. Pass '' on update to clear."
                }
            },
            "required": ["action"]
        })
