...
//!    (`$TMPDIR/apexrouter-results/<tool_use_id>.txt`) and the in-context
...
//!   (`std::env::temp_dir().join("apexrouter-results")`) which matches the
//!   default Python branch (`STORAGE_DIR = "/tmp/apexrouter-results"`) on
//!   Linux/macOS and uses `%TEMP%\\apexrouter-results` on Windows. A
...
pub const STORAGE_SUBDIR: &str = "apexrouter-results";
...
/// `std::env::temp_dir().join("apexrouter-results")`; tests inject a
/// tempdir-rooted override.
#[derive(Debug, Clone)]
pub struct StorageDir(pub PathBuf);

impl StorageDir {
    /// OS default — `$TMPDIR/apexrouter-results` (or `/tmp/apexrouter-results`
...
mod tests {
    ...
    fn make_storage(tmp: &TempDir) -> StorageDir {
        StorageDir(tmp.path().join("apexrouter-results"))
    }
