# ApexRouter Plugin Authors Guide

ApexRouter exposes a stable plugin API that lets third parties contribute tools, hooks, providers, skills, rules, MCP servers, and user-model backends without forking the engine. This guide is the canonical onboarding doc for v0.6.5. Read it once end-to-end before you write code; come back later for the manifest and permission reference tables.
...
Every ApexRouter plugin runs in exactly one of four modes. Pick by answering these in order:

```
Are you on ApexRouter's first-party team and shipping with the engine binary?
...
- **mcp-bridge** — Manifest-only wrapper around an existing MCP server. ApexRouter synthesizes a `PluginTool` per discovered MCP tool. Zero plugin code; the MCP server you wrap is the implementation. Use this whenever someone has already built the integration in MCP — you do not need to port it.
...
This means: do not name your plugin after a capability ("my-tool-plugin"). Name it after the vendor or feature boundary ("apexrouter-spotify", "apexrouter-honcho"). The capabilities are an implementation detail.
...
[plugin]                                                # see line 251
name        = "apexrouter-spotify"
version     = "0.1.0"
description = "Spotify control surface as a ApexRouter tool"
entry       = "apexrouter_spotify::plugin"                 # static: Rust path; wasm: ignored
...
binary_path = "./apexrouter-spotify-bin"
args        = ["--mode=stdio"]
...
- Files in `~/.apexrouter/trusted-keys/*.pub`. The default directory is anchored at `dirs::home_dir().join(".apexrouter").join("trusted-keys")` (`crates/wcore-agent/src/plugins/sig_verifier.rs:55-63`). Override via `APEXROUTER_CLI_TRUSTED_KEYS_DIR` for tests.
...
A signed plugin ships a sidecar file `<plugin_dir>/apexrouter-plugin.sig` containing a raw 64-byte ed25519 signature over the plugin payload. The filename is the constant `PLUGIN_SIG_FILENAME` (`crates/wcore-agent/src/plugins/sig_verifier.rs:26`).
...
Patch and minor bumps stay compatible as long as the WIT files do not break. **WIT files are semver.** Major changes to any `apexrouter:host@x.y.z` package require a manifest `plugin_api_version` bump and a coordinated migration. Treat them like a public API contract.
...
RUST_LOG=wcore_plugin_wasm=debug,wcore_agent::plugins=debug \
  APEXROUTER_CLI_PLUGIN_TRUST_UNSIGNED=1 \
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{}}' \
  | ./apexrouter-spotify-bin --mode=stdio
```

If it does not echo a well-formed response, the engine will not load it either. Debug it like any other CLI.

**mcp-bridge.** All existing MCP debugging tooling applies — `mcp-inspector`, JSON-RPC tracing, server-side stdout. If the underlying MCP server works in any MCP client, it works in ApexRouter.
...
```bash
cargo generate --git https://github.com/APEX-AI-LABS-LLP/apexrouter-cli \
...
Drop the resulting `.wasm` plus a signed `apexrouter-plugin.sig` into `~/.apexrouter/plugins/my-wasm-plugin/`.

**Wrap an MCP server in 60 seconds:**

```bash
mkdir -p ~/.apexrouter/plugins/mcp-time
cp examples/plugin-subprocess-mcp/plugin.toml ~/.apexrouter/plugins/mcp-time/
