...
ApexRouter-CLI's memory layer is a **5-partition × 3-tier** SQLite-backed
...
ApexRouter-CLI agents are long-lived: one user, many projects, many
...
| Project   | `Tier::Project` | Per project root, in `.apexrouter-cli/memory/memory.db`.         |
...
(`.apexrouter-cli.toml` for project scope, or the global config — see
...
| Path                                                       | Contents                                  |
|------------------------------------------------------------|-------------------------------------------|
| `<project_root>/.apexrouter-cli/memory/memory.db`            | Project-tier SQLite                       |
...
`<config_dir>` follows OS convention (`~/.config/apexrouter-cli` on Linux,
`~/Library/Application Support/apexrouter-cli` on macOS, `%APPDATA%\apexrouter-cli`
on Windows).

Override the base directory by exporting `WCORE_MEMORY_DIR` before
launch. When set, it overrides the default platform config directory.
The resolution order lives in
...
### CLI: `apexrouter-cli --memory-show <session>`

The fastest way to see what the agent remembers about a session is the
inspection flag (shipped in M3.4):

```bash
$ apexrouter-cli --memory-show 2026-05-16-abc123 --project-dir ~/project
{
...
```bash
sqlite3 .apexrouter-cli/memory/memory.db ".schema episodes"
sqlite3 .apexrouter-cli/memory/memory.db \
...
project's `.apexrouter-cli.toml`. To opt in globally, add it to the
global config (`<config_dir>/apexrouter-cli/config.toml`). Project values
...
```bash
rm .apexrouter-cli/memory/memory.db
```

To wipe global-tier memory (including the user model):

```bash
rm -rf "$(apexrouter-cli --print-config-dir)/memory"
```
