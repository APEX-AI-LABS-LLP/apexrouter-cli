# apexrouter-e2e-windows-soak.ps1
#
# Cross-OS counterpart to scripts/apexrouter-e2e-real-workload.sh — runs the
...
#   pwsh scripts/apexrouter-e2e-windows-soak.ps1
#
...
if ($args.Count -gt 0 -and ($args[0] -eq "--help" -or $args[0] -eq "-h")) {
    ...
    @'
apexrouter-e2e-windows-soak.ps1 -- full ApexRouter-CLI workload on Windows (~15-30min)
...
WHAT IT DOES
  ...
  - Captures all logs to LOCAL_RESULTS_DIR (default: $env:TEMP\apexrouter-windows-soak-<RUN_ID>)

ASSUMES
  - cargo, cargo-nextest, cargo-mutants are on PATH
    (the GHA workflow installs them via taiki-e/install-action)
  - Workspace is at the current directory or $env:APEXROUTER_CLI_REPO_ROOT

ENV OVERRIDES
  APEXROUTER_CLI_REPO_ROOT=<path>  default: $PWD
  LOCAL_RESULTS_DIR=<path>  default: $env:TEMP\apexrouter-windows-soak-<RUN_ID>
'@ | Out-Host
    exit 0
}
...
$RepoRoot = if ($env:APEXROUTER_CLI_REPO_ROOT) { $env:APEXROUTER_CLI_REPO_ROOT } else { (Get-Location).Path }
$ResultsDir = if ($env:LOCAL_RESULTS_DIR) { $env:LOCAL_RESULTS_DIR } else { Join-Path $env:TEMP "apexrouter-windows-soak-$RunId" }
...
# Verify binary exists. Windows MSVC target produces apexrouter-cli.exe.
$BinaryCandidates = @(
    (Join-Path $RepoRoot "target\release\apexrouter-cli.exe"),
    (Join-Path $RepoRoot "target\release\apexrouter-cli")
)
$Binary = $BinaryCandidates | Where-Object { Test-Path $_ } | Select-Object -First 1
if (-not $Binary) {
    Write-Fail "apexrouter-cli binary not found at expected target/release/ location"
