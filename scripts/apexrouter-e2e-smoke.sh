...
#
# Run: scripts/apexrouter-e2e-smoke.sh
...
if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
  cat <<'__HELP_END__'
apexrouter-e2e-smoke.sh — DO ephemeral-runner control-plane smoke test (~90s, ~$0.0001)
...
REQUIRES
  Env file at $APEXROUTER_CLI_E2E_ENV (default ~/.config/apexrouter-e2e/do.env)
...
DROPLET_NAME_PREFIX="apexrouter-e2e-smoke"
...
# ---------------------------------------------------------------------------
DROPLET_ID=""
TMPDIR_RUN=$(mktemp -d -t apexrouter-e2e-smoke-XXXXXX)
...
if [ ! -r "$ENV_FILE" ]; then
  ...
  echo "  set APEXROUTER_CLI_E2E_ENV to override the path"
  exit 1
fi
...
print(json.dumps({
  ...
  'tags': ['apexrouter-e2e', 'smoke-test'],
...
ssh-keygen -lf /root/.ssh/deploy_apexrouter_cli.pub 2>/dev/null \
  || ssh-keygen -yf /root/.ssh/deploy_apexrouter_cli | ssh-keygen -lf - -E sha256
echo "--- git ls-remote via deploy key (proves end-to-end clone path) ---"
git ls-remote git@github.com:APEX-AI-LABS-LLP/apexrouter-cli.git HEAD refs/heads/main 2>&1
echo "--- SUCCESS ---"
'
