#!/usr/bin/env bash
# apexrouter-e2e-real-workload.sh
#
# REAL end-to-end validation of ApexRouter-CLI on the DO ephemeral runner.
...
#
# Run: scripts/apexrouter-e2e-real-workload.sh
...
if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
  cat <<'__HELP_END__'
apexrouter-e2e-real-workload.sh — full ApexRouter-CLI workload on a DO droplet (~25-40min)
...
WHAT IT DOES
  ...
  · Captures all logs to /tmp/apexrouter-e2e-real-<timestamp>/
  · Destroys the droplet (trap-guaranteed)

ENV OVERRIDES
  DROPLET_SIZE=<slug>            default: s-4vcpu-8gb (c-4 not in nyc3)
  APEXROUTER_CLI_E2E_ENV=<path>         default: ~/.config/apexrouter-e2e/do.env
  LOCAL_RESULTS_DIR=<path>       default: /tmp/apexrouter-e2e-real-<RUN_ID>
...
# Config
# ---------------------------------------------------------------------------
ENV_FILE="${APEXROUTER_CLI_E2E_ENV:-$HOME/.config/apexrouter-e2e/do.env}"
DROPLET_NAME_PREFIX="apexrouter-e2e-real"
...
LOCAL_RESULTS_DIR="${LOCAL_RESULTS_DIR:-/tmp/apexrouter-e2e-real-${RUN_ID}}"
...
#cloud-config
write_files:
  - path: /root/.ssh/deploy_apexrouter_cli
    permissions: '0600'
    owner: root:root
    encoding: b64
  ...
  - path: /root/.ssh/config
    ...
    content: |
      Host github.com
        IdentityFile /root/.ssh/deploy_apexrouter_cli
        IdentitiesOnly yes
...
print(json.dumps({
  ...
  'tags': ['apexrouter-e2e', 'real-workload'],
...
# ---------------------------------------------------------------------------
phase "PHASE E — clone apexrouter-cli at v0.8.3"

ssh "${SSH_ARGS[@]}" "root@$DROPLET_IP" "bash -s" <<'EOREMOTE' 2>&1 | tee "$LOCAL_RESULTS_DIR/E-clone.log"
set -e
cd /root
git clone --depth=1 --branch v0.8.3-apexrouter-hardened-followup git@github.com:APEX-AI-LABS-LLP/apexrouter-cli.git wcore
cd wcore
git log -1 --oneline
...
echo "===> verify binary"
ls -la target/release/apexrouter-cli
file target/release/apexrouter-cli
echo "===> binary executes"
./target/release/apexrouter-cli --version
echo "===> binary --help"
./target/release/apexrouter-cli --help 2>&1 | head -30
EOREMOTE

ok "apexrouter-cli builds + executes on Linux"
